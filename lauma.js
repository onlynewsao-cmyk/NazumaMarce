...
const directory = `./DADOS_SYSTEM/anti_sp.json`;
...
async function startSystemDark() {

module.exports = SystemDark = async(upsert, SystemDark, qrcode) => {
...
const VRF_JSON_GRUPO = fs.existsSync(`./DADOS_SYSTEM/grupos/ATIVAÇÕES-SystemDark/${from}.json`) ? true : false;

if(VRF_JSON_GRUPO) {
var jsonGp = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/ATIVAÇÕES-SystemDark/${from}.json`));
}
...
await SystemDark.relayMessage(from, detalhes, {});
}
}
break;
...
if(visualizarmsg) {
await SystemDark.readMessages([info.key]);
} else {
...
if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/ATIVAÇÕES-SystemDark/${from}.json`) && jsonGp[0].multiprefix) {
var prefix = jsonGp[0]?.prefixos[jsonGp[0]?.prefixos?.indexOf(String(body)?.trim()?.charAt(0))] || jsonGp[0].prefixos[0]
}

if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/ATIVAÇÕES-SystemDark/${from}.json`) && !jsonGp[0].multiprefix) {var prefix = setting.prefix} else if(!isGroup) {var prefix = setting.prefix};
...
try {var groupMetadata = isGroup ?await SystemDark.groupMetadata(from): ""} catch {return}

const groupName = isGroup ? groupMetadata.subject : '';

const botNumberLID = SystemDark?.user?.lid?.split(':')[0] + '@lid' || '';

const sender = jidNormalizedUser(isGroup ? info?.key?.participantPn || info?.key?.senderPn || await SystemDark?.user?.id || info?.key?.participant : info?.key?.senderPn || info?.key?.participant ||info?.key?.remoteJid);

if (sender.includes('@lid') && SystemDark?.user?.lid) {
sender = SystemDark.user.id?.split(':')[0] + '@s.whatsapp.net' || sender;
}

const botNumber = SystemDark.user.id.split(':')[0]+'@s.whatsapp.net';
const messagesC = PR_String.slice(0).trim().split(/ +/).shift().toLowerCase();

const NumeroDoBot = SystemDark.user.id.split(':')[0];

const itsMe = info.key.fromMe || (SystemDark?.user?.lid ? info?.key?.participant === SystemDark.user.lid.split(':')[0] + '@lid' : false);
...
const isSystemDarkIA = isGroup ? dataGp[0].systemDarkIA : undefined;

const isAutorepo = isGroup ? dataGp[0].autoresposta : undefined

const isModobn =isGroup ? dataGp[0].jogos : undefined

const isSystemDarkEscuta = isGroup ? dataGp[0].SystemDarkescutando : undefined
...
const isAntiPay = isGroup ? dataGp[0].antipay : undefined;

const isAntiBot = isGroup ? dataGp[0].antibot : false;
...
return SystemDark.sendMessage(from,{document: fs.readFileSync("./DADOS_SYSTEM/selo.xlsx"),
mimetype: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
fileName: `${NomeDoBot}.xlsx`,
fileLength: 11811160064,
caption: text,
contextInfo: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/menu.mp3');
await SystemDark.sendMessage(from, {
audio: soundft,
mimetype: "audio/mpeg",
...
await SystemDark.sendMessage(id, { text: textCaption, contextInfo: { externalAdReply: { title: title, body: desc, thumbnail: await getBuffer(imageUrl), mediaType: 1, sourceUrl: linkAcess } } }, { contextInfo: { ...DLchanneldl } }, { quoted: quotedThis })
}

async function replyWithNewsletter(text, options = {}, quotedThis = info) {
await SystemDark.sendMessage(from, {text: text, contextInfo: options}, {quoted: quotedThis})
.catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}

async function loadBaileysExtras() {
return true;
}

async function replyWithReaction(text, options = {}, quotedThis = info) {
await SystemDark.sendMessage(from, options)
await SystemDark.sendMessage(from, {text: text}, {quoted: quotedThis})
.catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}
...
await SystemDark.sendMessage(ChatID, { sticker: { url: FileN } }, { contextInfo: { ...DLchanneldl } }, { quoted: selo }
)
.catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
...
await SystemDark.sendMessage(ChatID, { image: { url: FileN } }, { contextInfo: { ...DLchanneldl } }, { quoted: selo }
).catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
})
}
...
await SystemDark.sendMessage(ChatID, { text: texto }, { contextInfo: { ...DLchanneldl } }
).catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}
...
await SystemDark.sendMessage(local, {text: teks.trim(), contextInfo:{...DLchanneldl, mentionedJid: memberr}}, {quoted: selo}).catch(async(e) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}

const mentions = async(teks = '', mb, id) => {
(id == null || id == undefined || id == false) ? await SystemDark.sendMessage(from, {text: teks.trim(), mentions: mb}) : await SystemDark.sendMessage(from, {text: teks.trim(), contextInfo:{...DLchanneldl, mentionedJid: mb}}, {quoted: selo})
}
...
await SystemDark.sendMessage(from, {text: teks.trim(), contextInfo:{...DLchanneldl, mentionedJid: memberr}}, {quoted: selo}).catch(async(e) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}
...
await SystemDark.sendMessage(from, {image: {url: FileN}, caption: teks.trim(), mentions: memberr}, {quoted: thisQuoted}).catch(async(error) => {
await SystemDark.sendMessage(from, {text: mess.error()}, {quoted: selo});
});
}

const reagir = async (idgp, emj) => {
await SystemDark.sendMessage(idgp, {react: {text: emj, key: info.key}});
}

const verificarN = async(sla) => {
const [result] = await SystemDark.onWhatsApp(sla)
if(result == undefined) {
...
await SystemDark.sendMessage(from, {text: mess.markingAllMember()}, {quoted: selo});
if(IS_DELETE) {
setTimeout(async() => {
await SystemDark.sendMessage(from, {delete: {remoteJid: from, fromMe: false, id: info.key.id, participant: sender}});
}, 500);
}
SystemDark.groupParticipantsUpdate(from, [sender], "remove");
}
}
...
let encmediaa = await sendImageAsSticker2(SystemDark, from, owgi, selo, { packname:pack, author:author2})
DLT_FL(encmediaa)
}
...
let encmedia = await sendVideoAsSticker2(SystemDark, from, owgi, selo, { packname:pack, author:author2})
DLT_FL(encmedia)
}
} 
}, 1000)
...
SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.message.extendedTextMessage.contextInfo.stanzaId, participant: menc_prt}})
setTimeout(async() => {
SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender}})
}, 0)
}
}
...
await SystemDark.sendMessage(to, {sticker: media}, {sendEphemeral: true, contextInfo: { forwardingScore: 50, isForwarded: true}, quoted: selo}).catch(async(error) => {
...
chalk.greenBright(
`╭┈ׅׄ┉━━═✯𑁍🪷𑁍✯═━━┉ֹׅ┈╮
┃֪࣪𝆺𝅥 ${isGroup ? '👥 𝐌𝐄𝐍𝐒𝐀𝐆𝐄𝐌 𝐍𝐎 𝐆𝐑𝐔𝐏𝐎' : '👤 𝐌𝐄𝐍𝐒𝐀𝐆𝐄𝐌 𝐍𝐎 𝐏𝐑𝐈𝐕𝐀𝐃𝐎'}
┃֪࣪𝆺𝅥👤 𝐔𝐒𝐔𝐀́𝐑𝐈𝐎 ➥ ${user}
┃֪࣪𝆺𝅥📱 𝐍𝐔́𝐌𝐄𝐑𝐎 ➥ ${number}
┃֪࣪𝆺𝅥💬 𝐂𝐇𝐀𝐓 ➥ ${chatType} ${groupInfo}
┃֪࣪𝆺𝅥📨 𝐓𝐈𝐏𝐎 ➥ ${msgType}
┃֪࣪𝆺𝅥📝 𝐂𝐎𝐍𝐓𝐄𝐔́𝐃𝐎 ➥ ${msgContent}
╰┈ׅׄ┉━━═✯💮✯═━━┉ֹׅ┈╯
`
)
);
...
fs.writeFileSync('./DADOS_SYSTEM/usuarios/joguinhodavelha.json', JSON.stringify(joguinhodavelhajs))
joguinhodavelhajs2.splice([])
fs.writeFileSync('./DADOS_SYSTEM/usuarios/joguinhodavelha2.json', JSON.stringify(joguinhodavelhajs2))
}
}
}
...
fs.writeFileSync("./DADOS_SYSTEM/func/namoro1.json", JSON.stringify(namoro1));
...
fs.writeFileSync("./DADOS_SYSTEM/func/namoro2.json", JSON.stringify(namoro2));
let ppimg;
try {
ppimg = await SystemDark.profilePictureUrl(sender, 'image');
} catch {
ppimg = imgperfil;
}
await SystemDark.sendMessage(from, {
...
let C14 = namoro2.map(i => i.id).indexOf(sender);
let C12 = namoro1.map(i => i.usu1).indexOf(`${namoro2[C14].pedido}@s.whatsapp.net`);
await SystemDark.sendMessage(from, {text: mess.fora(namoro1, C12, sender, prefix), contextInfo: {...DLchanneldl, mentionedJid: [namoro1[C12].usu1, sender]}}, { quoted: selo });
...
const CompareLink = await SystemDark.groupInviteCode(from);
...
const getImage = await SystemDark.profilePictureUrl(sender, 'image').catch(() => imgperfil);
const getbio = `${number}@s.whatsapp.net`;

let recadoW;
try {
const recadoUser = await SystemDark.fetchStatus(getbio);
...
await getdados()
.then(async (lm) => {
await SystemDark.sendMessage(ANT_SP.groupId, {
text: mess.antisp(lm),
...
SystemDark.groupSettingUpdate(from, 'announcement')
setTimeout(() => {
SystemDark.groupSettingUpdate(from, 'not_announcement')
}, 1200)
...
if(isBot) return 
if(!JSON.stringify(groupMembers).includes(sender)) return
await SystemDark.groupParticipantsUpdate(from, [sender], 'remove');
await SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender } });
}
if (info.message?.requestPaymentMessage || info.message?.sendPaymentMessage) {
await SystemDark.groupSettingUpdate(from, 'announcement');
await SystemDark.groupParticipantsUpdate(from, [sender], 'remove');
await SystemDark.groupSettingUpdate(from, 'not_announcement');
}
}
...
  fs.writeFileSync("./DADOS_SYSTEM/grupos/muted.json", JSON.stringify(muted, null, 2));
}
...
fs.writeFileSync('./DADOS_SYSTEM/INFO_SYSTEM/media/countmsg.json', JSON.stringify(countMessage));
...
fs.writeFileSync("./DADOS_SYSTEM/usuarios/users.json", JSON.stringify(pushnames, null, 2));
}
...
const SystemDarkAluguel = './ARQUIVES/tictactoe/grupos_com_aluguel.json'
let gruposAutorizados = []
if (!fs.existsSync(SystemDarkAluguel)) fs.writeFileSync(SystemDarkAluguel, JSON.stringify([]))
gruposAutorizados = JSON.parse(fs.readFileSync(SystemDarkAluguel))
function salvarGrupos() {
fs.writeFileSync(SystemDarkAluguel, JSON.stringify(gruposAutorizados, null, 2))
}
...
salvarGrupos()
const meta = await SystemDark.groupMetadata(from)
...
await SystemDark.sendMessage(from, msg)
} catch (e) {
...
await SystemDark.sendMessage(from, { text: "Por favor, envie uma imagem com legenda." }, { quoted: selo });
if (IS_DELETE) {
setTimeout(async () => {
if (groupMembers.find(member => member.id === sender)) {
await SystemDark.groupParticipantsUpdate(from, [sender], 'remove');}}, 1000); }}}
...
if(info.key.fromMe) return
if(isGroupAdmins) return await SystemDark.sendMessage(from, {text: mess.messageProhibitedDetAdmin()}, {quoted: selo})
if(IS_DELETE) {
setTimeout(async() => {
await SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender}})
}, 500)
...
if(Procurar_String.includes("chat.whatsapp.com")) {
link_dgp = await SystemDark.groupInviteCode(from)
...
SystemDark.groupParticipantsUpdate(id, [usu], "remove")}
//======(ANTI-VIDEO)========\\
if(isAntiVid && isBotGroupAdmins && type == 'videoMessage') {
if(isGroupAdmins) return await SystemDark.sendMessage(from, {text: mess.messageProhibitedDetAdmin()}, {quoted: selo});
if(dataGp[0].legenda_video == "0") {
await SystemDark.sendMessage(from, {text: mess.messageProhibitedDetUser()}, {quoted: selo});
} else {
await SystemDark.sendMessage(from, {text: dataGp[0].legenda_video}, {quoted: selo});
}
if(IS_DELETE) {
setTimeout(async() => {
await SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender}});
...
SystemDark.ws.on('CB:call', async (B) => {
if (B.content[0].tag == 'offer') {
const numero = B.content[0].attrs['call-creator'];
await SystemDark.updateBlockStatus(numero, "block");
}
});
}
...
setTimeout(async () => {
await SystemDark.updateBlockStatus(sender, 'block')
}, 2000)
}
...
const RG_SCOINS = JSON.parse(fs.readFileSync("./DADOS_SYSTEM/func/coins.json"));
...
fs.writeFileSync("./DADOS_SYSTEM/func/coins.json", JSON.stringify(index, null, 2) + "\n")
}
...
if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`)){
let dataA = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`))

if(budy.slice(0,4).toUpperCase() == dataA.palavraOriginal.slice(0,4).toUpperCase() && budy.toUpperCase() != dataA.palavraOriginal) {
...
await SystemDark.sendMessage(from, {
text: mess.acert(pushname, dataA, budy),
mentions: [sender]
}, {quoted: selo})

await SYSTEM_COIN.AdicionarCoins(sender, 5);

fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`)

setTimeout(async() => {
fs.writeFileSync(
`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`,
`${JSON.stringify(palavrasANA[Math.floor(Math.random() * palavrasANA.length)])}`
)

dataAB = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`))

SystemDark.sendMessage(from, {
text: mess.anagrama(shuffle, dataAB)
}, {quoted: selo})
...
if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`)){
let dQ = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`))
...
await SystemDark.sendMessage(from,{text: mess.quizaC(pushname, dQ)}, {"mentionedJid": [sender]}, {quoted: selo}) 
await SYSTEM_COIN.AdicionarCoins(sender, 5);
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`)		
setTimeout(async() => {
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`, `${JSON.stringify(quizanimais[Math.floor(Math.random() * quizanimais.length)])}`)
dataQA = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`))
wew = await getBuffer(dataQA.foto)
await SystemDark.sendMessage(from, {image: wew, caption: `🤔 Pergunta: ${dataQA.question}`}, {quoted: selo})
}, 5000)
}
}

if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`)){
let dQF = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`))
...
await SystemDark.sendMessage(from,{text: mess.quizC(pushname, dQF)}, {"mentionedJid": [sender]}, {quoted: selo}) 
await SYSTEM_COIN.AdicionarCoins(sender, 5);
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`)		
setTimeout(async() => {
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`, `${JSON.stringify(quizFutebol[Math.floor(Math.random() * quizFutebol.length)])}`)
dataQF = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`))
await SystemDark.sendMessage(from, {text: mess.quizfut(dataQF)}, {quoted: selo})
}, 5000)
}
}

if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`)){

let perg_gartic = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`))

if(budy.slice(0,4).toUpperCase() == perg_gartic.resposta.slice(0,4).toUpperCase() && budy.toUpperCase() != perg_gartic.resposta) {
return reply('Está perto!')
}

if(budy.toUpperCase() == perg_gartic.resposta) {

const coinsGanhos = Math.floor(Math.random() * 10) + 1;

let descorbert = `𝐏𝐀𝐑𝐀𝐁𝐄́𝐍𝐒 
ᴜsᴜᴀ́ʀɪᴏ: @${pushname}, ᴠᴏᴄᴇ̂ ᴀᴄᴇʀᴛᴏᴜ ᴏ ᴅᴇsᴇɴʜᴏ! ʀᴇᴄᴏᴍᴘᴇɴsᴀ: +${coinsGanhos} ᴄᴏɪɴs`
...
await SYSTEM_COIN.AdicionarCoins(sender, coinsGanhos);

fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`);

setTimeout(async() => {

fs.writeFileSync(
`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`,
`${JSON.stringify(garticArchives[Math.floor(Math.random() * garticArchives.length)])}`
)

let dataGartic2 = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`))
...
if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`)){
let enigmaData = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`))
...
SystemDark.sendMessage(from,{text: mess.respostaE(pushname)}, {"mentionedJid": [sender]}, {quoted: selo})
await SYSTEM_COIN.AdicionarCoins(sender, 5);
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`)	
setTimeout(async() => {
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`, `${JSON.stringify(enigmaArchive[Math.floor(Math.random() * enigmaArchive.length)])}`)
enigmaD = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`))
enigmaTezt = mess.respondeE(enigmaD, prefix)
wew = await getBuffer(rvenigma)
await SystemDark.sendMessage(from, {image: wew, caption: enigmaTezt}, {quoted: selo})
}, 5000)
}
}

if(isGroup && fs.existsSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`)){
whatMusic = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`))
...
SystemDark.sendMessage(from,{text: mess.whatmusic(whatMusic, pushname)}, {"mentionedJid": [sender]}, {quoted: selo}); fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`);
setTimeout(async() => {
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`, `${JSON.stringify(whatMusicAr[Math.floor(Math.random() * whatMusicAr.length)])}`)
wmusic = JSON.parse(fs.readFileSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`))
textM = mess.wmusic(wmusic, II)
await SystemDark.sendMessage(from, {text: textM}, {quoted: selo})
}, 5000)
}
}
...
await SystemDark.sendMessage(from, px, { quoted: selo });
}
...
await SystemDark.sendMessage(from, msg, { quoted: selo });
} catch (e) {
console.error(e);
await SystemDark.sendMessage(from, { text: caption, contextInfo: { ...DLchanneldl } }, { quoted: selo });
}
}
...
const testeFoto = await axios.get(
`${API_URL}/api/tiktok-foto?url=${encodeURIComponent(url)}&apikey=${API_KEY_SYSTEM_DARK}&index=0`,
{
...
const totalFotos = parseInt(testeFoto.headers["x-total-fotos"]) || 0;
...
image: {
url: `${API_URL}/api/tiktok-foto?url=${encodeURIComponent(url)}&apikey=${API_KEY_SYSTEM_DARK}&index=${i}`
},

text: `📸 Foto ${i + 1} de ${totalFotos}`,
...
await SystemDark.sendMessage(from, {
video: {
url: `${API_URL}/api/tiktok-video?url=${encodeURIComponent(url)}&apikey=${API_KEY_SYSTEM_DARK}`
},
mimetype: "video/mp4",
...
await SystemDark.sendMessage(from, {
video: { url: `${API_WAGURI}/youtube-video?url=${url}&apitoken=${KEY_WAGURI}`},
mimetype: "video/mp4",
fileName: `video.mp4`,
contextInfo: DLchanneldl
}, { quoted: selo });
...
// 🤖 SystemDark IA — controle por grupo
// 🤖 SystemDark IA — modo automático (igual autodown)

const totalhit = JSON.parse(fs.readFileSync('./DADOS_SYSTEM/data/totalcmd.json'))
const CmdUsedBOT = () => {
totalhit[0].totalcmd += 1
fs.writeFileSync('./DADOS_SYSTEM/data/totalcmd.json', JSON.stringify(totalhit))
}
if (isCmd) CmdUsedBOT()

const reqcmd = JSON.parse(fs.readFileSync('./DADOS_SYSTEM/data/totalcmd.json'))
...
fs.writeFileSync("./DADOS_SYSTEM/func/limitarcmd.json", JSON.stringify(Limit_CMD, null, 2));
...
SystemDark.sendImageAsSticker = async (jid, path, options = {}) => {
...
await SystemDark.sendMessage(jid, { sticker: { url: buffer }, ...options})
return buffer
}

SystemDark.sendInteractiveTXT = async(idChat, mainText = '', footerText = '', quotedMessage = {}, buttonsParams = {}) => {
try {
await SystemDark.relayMessage(idChat, {interactiveMessage: {body: {text: mainText}, footer: {text: footerText}, contextInfo: quotedMessage, nativeFlowMessage: buttonsParams}}, {});
} catch(errorMessage) {
await reply(String(errorMessage));
}
}

SystemDark.sendInteractiveIMG = async(imageDir, ChatID, definedText = '', footer = '', contextMessageInfo = {}, buttonsParams = {}) => {
try {
create = await prepareWAMessageMedia({image: fs.readFileSync(imageDir)}, {upload: SystemDark.waUploadToServer});
imageCreate = create.imageMessage;
await SystemDark.relayMessage(ChatID, {interactiveMessage: {header: {hasMediaAttachment: true, imageMessage: imageCreate}, headerType: 'IMAGE', body: {text: definedText}, footer: {text: footer}, contextInfo: contextMessageInfo, nativeFlowMessage: buttonsParams}}, {});
} catch(errorMessage) {
return reply(String(errorMessage));
 ...
 const getallcases = () => {
findindex = fs.readFileSync("SystemDark.js").toString().match(/case\s+'(.+?)'/g)
cstt = []
for(i of findindex) {
cstt.push(i.split(`'`)[1])
}
...
const SystemDarksticker = Object.keys(info.message)[0] == "stickerMessage" ? info.message.stickerMessage.fileSha256.toString('base64') : ""

if (SystemDarksticker) {
const comandoFig = getComandoFig(SystemDarksticker)
...
const NoPrefixSystemDark = budy2.trim().slice(0).trim().split(" ")[0].trim().toLocaleLowerCase()

const comandoRegistrado = getComandoNoPrefix(NoPrefixSystemDark)
...
if (!global.controlePin) global.controlePin = {};
...
///////////////////////ÍNICIO DE TUDO\\\\\\\\\\\\\\\\\\\
// === SYSTEM DARK - MULTIVERSO ANIME RPG (COMBATE NUMÉRICO 0 a 4) ===
if (!isCmd && /^[0-4]$/.test(body?.trim())) {
  const rpgTurn = await handleRpgCommand(body.trim(), [], sender, reply, null, pushname || "Caçador");
  if (rpgTurn !== null) return;
}

switch(command) {
      // === SYSTEM DARK - MULTIVERSO ANIME RPG (17+ MÓDULOS) ===
      case 'perfilrpg':
      case 'perfil':
      case 'carteira':
      case 'banco':
      ...
      case 'top': {
        ...
        const handled = await handleRpgCommand(command, args, sender, reply, (info.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0] || null), pushname || "Caçador");
      ...
      case 'menurpg':
      case 'rpg': {
        const menuTxt = await menurpg(prefix, NomeDoBot, pushname || "Caçador");
        await reply(menuTxt);
        break;
      }
//=============[ SYSTEM FIGURINHA ]===========\\
...
const caminhoVideo = './DADOS_SYSTEM/INFO_SYSTEM/LOGOS/dono.mp4';
...
if(!SoDono) return reply(mess.onlyOwner())
	...
	if (!nescessario.aluguel) {
		...
		fs.writeFileSync(`./DADOS_SYSTEM/INFO_SYSTEM/media/nescessario.json`, JSON.stringify(nescessario, null, 2) + "\n");
...
try {
const meta = await SystemDark.groupMetadata(g.id)
const nome = meta.subject || '🕯️ ɢʀᴜᴘᴏ sᴇᴍ ɴᴏᴍᴇ'
const membros = meta.participants.length || 0
const inviteCode = await SystemDark.groupInviteCode(g.id).catch(() => null)
...
case 'ngl':
case 'sendngl': // dylan modzPetrøv 
try {
if (!q.includes("|")) {
return reply(`*ᴜꜱᴀ ᴀꜱꜱɪᴍ: ${prefix}ɴɢʟ ʟɪɴᴋ | ᴍᴇɴꜱᴀɢᴇᴍ | ǫᴜᴀɴᴛɪᴅᴀᴅᴇ* 🙇‍♂️`)
}
let partes = q.split("|").map(x => x.trim()) // dylan modzPetrøv 
if (partes.length < 3) {
...
username: username,
question: mensagem,
deviceId: deviceId // dylan modzPetrøv 
}, {
...
try {
const groupMetadata = await SystemDark.groupMetadata(from);
...
const isBot = jid === SystemDark.user.id;
...
await SystemDark.groupParticipantsUpdate(from, estrangeiros, 'remove');
...
if (totalFake > 0) {
if (isGroupAdmins) {
SystemDark.sendMessage(from, { text: txtFake, mentions: mencionadosFake }, { quoted: selo });
} else reply(txtFake);
...
if (totalBr > 0) {
if (isGroupAdmins) {
SystemDark.sendMessage(from, { text: txtBr, mentions: mencionadosBr }, { quoted: selo });
} else reply(txtBr);
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if (!isBotGroupAdmins) return reply(mess.onlyBotAdmin());
const pendentes = await SystemDark.groupRequestParticipantsList(from);
const total = pendentes?.length || 0;
if (total === 0)
return reply('*💁‍♂️ | ɴᴀᴏ ʜᴀ ɴᴇɴʜᴜᴍᴀ sᴏʟɪᴄɪᴛᴀᴄᴀᴏ ᴘᴇɴᴅᴇɴᴛᴇ ɴᴏ ɢʀᴜᴘᴏ. 🤷‍♂️*');
for (const p of pendentes) {
await SystemDark.groupRequestParticipantsUpdate(from, [p.jid], 'approve');
}
...
for (const p of pendentes) {
await SystemDark.groupRequestParticipantsUpdate(from, [p.jid], 'reject');
}
...
if (!isGroup) return reply(mess.onlyGroup());
if (!isGroupAdmins) return reply(mess.onlyAdmins());
const solAll = await SystemDark.groupRequestParticipantsList(from);
...
case 'entrar':
case 'entrargp': // dylan Modz eli emos
try { //começo do try
...
await SystemDark.groupAcceptInvite(URL_ID);
...
*APROVEITE O GRUPO!*`
SystemDark.sendMessage(from, {text: inff}, {quoted: selo})
break
...
/﹋<,︻╦╤─ ҉ - -----💥 
/﹋ 🅴 🅱🅴🅼 🆅🅸🅽🅳🅾 🆂🅴🆄🆂 🅵🅸🅻🅷🅾🆂 🅳🅰 🅿🆄🆃🅰`
SystemDark.sendMessage(from, {text: txtz}, {quoted: selo})
break
...
SystemDark.sendMessage(from, {text: bla}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {text: `*ᴄᴇʀᴛᴏ... ʀᴇᴍᴏᴠɪ ᴏ(ᴀ) @${menc_os2.split("@")[0]} ᴘᴏʀ ᴘᴏʀ ᴍᴏᴛɪᴠᴏꜱ ᴊᴜꜱᴛᴏꜱ 🙇‍♂️*`, contextInfo:{...DLchanneldl, mentionedJid: [menc_os2]}}, {quoted: selo}) 
await SystemDark.groupParticipantsUpdate(from, [menc_os2], "remove");
 } catch (e) {
console.error(e);
reply(mess.error());
 }
...
if(!menc_os2 || menc_jid2[1]) return reply("*🌟 ᴍᴇɴᴄɪᴏɴᴇ ᴀ ᴍᴇɴsᴀɢᴇᴍ ᴏᴜ ᴍᴀʀǫᴜᴇ ᴜᴍ ᴜsᴜᴀʀɪᴏ ᴄᴏᴍ ᴏ @ ᴅᴇʟᴇ*")
if(!JSON.stringify(groupMembers).includes(menc_os2)) return reply("*ᴇsᴛᴇ ᴜsᴜᴀʀɪᴏ sᴀɪᴜ ᴅᴏ ɢʀᴜᴘᴏ ᴏᴜ ғᴏɪ ʀᴇᴍᴏᴠɪᴅᴏ 🤷‍♂️*")
SystemDark.sendMessage(from, {text: `*@${menc_os2.split("@")[0]} ғᴏɪ ᴘʀᴏᴍᴏᴠɪᴅᴏ ᴀᴏ ᴄᴀʀɢᴏ ᴅᴇ ᴀᴅᴍ ᴄᴏᴍ sᴜᴄᴇssᴏ 🙇‍♂️*`, contextInfo:{...DLchanneldl, mentionedJid: [menc_os2]}}, {quoted: selo}) 
SystemDark.groupParticipantsUpdate(from, [menc_os2], "promote")
break
...
if(!menc_os2 || menc_jid2[1]) return reply("*🌟 ᴍᴇɴᴄɪᴏɴᴇ ᴀ ᴍᴇɴsᴀɢᴇᴍ ᴏᴜ ᴍᴀʀǫᴜᴇ ᴜᴍ ᴜsᴜᴀʀɪᴏ ᴄᴏᴍ ᴏ @ ᴅᴇʟᴇ*")
if(!JSON.stringify(groupMembers).includes(menc_os2)) return reply("*ᴇsᴛᴇ ᴜsᴜᴀʀɪᴏ sᴀɪᴜ ᴅᴏ ɢʀᴜᴘᴏ ᴏᴜ ғᴏɪ ʀᴇᴍᴏᴠɪᴅᴏ 🤷‍♂️*")
SystemDark.sendMessage(from, {text: `*@${menc_os2.split("@")[0]} ғᴏɪ ʀᴇʙᴀɪxᴀᴅᴏ ᴘᴀʀᴀ ᴍᴇᴍʙʀᴏ ᴄᴏᴍᴜᴍ ᴄᴏᴍ sᴜᴄᴇssᴏ 🙇‍♂️*`, contextInfo:{...DLchanneldl, mentionedJid: [menc_os2]}}, {quoted: selo}) 
SystemDark.groupParticipantsUpdate(from, [menc_os2], "demote")
break
...
fs.writeFileSync('./SystemDark.js', doc)
await SystemDark.sendMessage(from, {text: "*ᴀ ɪɴᴅᴇx ғᴏɪ ᴀᴛᴜᴀʟɪᴢᴀᴅᴀ ᴄᴏᴍ sᴜᴄᴇssᴏ ✅*"}, {quoted: selo})
} else {
...
fs.writeFileSync('./DADOS_SYSTEM/usuarios/flood.json', JSON.stringify({limitefl: q}, null, '\t'))
...
if (isAntiPay)
return reply(`*ᴏ ᴀɴᴛɪ ᴘᴀʏ ᴊᴀ́ ᴇsᴛᴀ́ ᴀᴛɪᴠᴏ.* 🤷‍♂️`);

dataGp[0].antipay = true;
setGp(dataGp);

return reply(`*ᴀɴᴛɪ ᴘᴀʏ ᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ.* ✅`);
...
if (!isAntiPay)
return reply(`*ᴏ ᴀɴᴛɪ ᴘᴀʏ ᴊᴀ́ ᴇsᴛᴀ́ ᴅᴇsᴀᴛɪᴠᴏ.* 🤷‍♂️`);

dataGp[0].antipay = false;
setGp(dataGp);

return reply(`*ᴀɴᴛɪ ᴘᴀʏ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ.* ✅`);

}

return reply(`*ᴜsᴇ 1 ᴘᴀʀᴀ ᴀᴛɪᴠᴀʀ ᴏᴜ 0 ᴘᴀʀᴀ ᴅᴇsᴀᴛɪᴠᴀʀ.* ❌`);
...
saveJSON(countMessage, ".INFO_SYSTEM/media/countmsg.json")
...
await limparPasta(`./DADOS_SYSTEM/qr-code`);
} catch(e) {
console.log(e);
...
setTimeout(async() => {
await SystemDark.groupParticipantsUpdate(from, [C2], "remove")
}, 6000)
break
...
case 'd_':
await SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.message.buttonsResponseMessage.contextInfo.stanzaId, participant: botNumber}})
break
...
await SystemDark.sendMessage(from, {text: teks, mentions: men}, {quoted: selo}).catch(async(error) => {
reply(mess.error())
})
break
...
if(!isBotGroupAdmins) return reply(mess.onlyBotAdmin())
blat = args.join(" ")
await SystemDark.groupUpdateSubject(from, `${blat}`)
await SystemDark.sendMessage(from, {text: 'Sucesso, alterou o nome do grupo.'}, {quoted: selo})
break
...
blabla = args.join(" ")
await SystemDark.groupUpdateDescription(from, `${blabla}`)
await SystemDark.sendMessage(from, {text: 'Sucesso, alterou a descrição do grupo'}, {quoted: selo})
break
...
medipp = rane 
SystemDark.updateProfilePicture(from, {url: medipp})
...
if(!isGroupAdmins) return reply(mess.onlyAdmins())
if(!isBotGroupAdmins) return reply(mess.onlyBotAdmin())
let metadata = await SystemDark.groupMetadata(from)
linkgc = await SystemDark.groupInviteCode(from)
...
if(!isGroupAdmins) return reply(mess.onlyAdmins())
if(!isBotGroupAdmins) return reply(mess.onlyBotAdmin())
await SystemDark.groupRevokeInvite(from)
...
fs.writeFileSync("./DADOS_SYSTEM/func/tmgroup.json", JSON.stringify(rgp))
...
case 'fzrtm':
if(!SoDono) return reply(mess.onlyOwner())
var rgp = JSON.parse(fs.readFileSync("./DADOS_SYSTEM/func/tmgroup.json"))
...
for (i = 0; i < rgp.length; i++) {
await SystemDark.sendMessage(rgp[i].id, DFC)}
break
...
let todosGrupos = await SystemDark.groupFetchAllParticipating()
...
SystemDarkMsg = { text: intro + red.replace(new RegExp(prefix + command, "gi"), ` `) }
} else if (!aud_d && !figu_d && green) {
SystemDarkMsg = { text: intro + green.replace(new RegExp(prefix + command, "gi"), ` `) }
} else if (figu_d) {
SystemDarkMsg = figu_d
figu_d.sticker = { url: figu_d.url }
} else if (aud_d) {
SystemDarkMsg = aud_d
aud_d.audio = { url: aud_d.url }
...
try {
let metadata = await SystemDark.groupMetadata(grupos[i])
...
await SystemDark.sendMessage(grupos[i], {
...SystemDarkMsg,
contextInfo
})
} catch (err) {
...
const metadata = await SystemDark.groupMetadata(from)
...
const aviso = await SystemDark.sendMessage(from, { 
text: `${q}`,
...
return await SystemDark.sendMessage(from, { text: args.join(" "), contextInfo: { mentionedJid: TDS_GP }})}
if (quotedMsg) {
...
return await SystemDark.sendMessage(from, { forward: { key: info.key, message: quotedMsg }, contextInfo: { mentionedJid: TDS_GP }})}
...
try {
const grupos = await SystemDark.groupFetchAllParticipating();
for (let idGrupo in grupos) {
await SystemDark.groupLeave(grupos[idGrupo].id);
}
...
try {
await SystemDark.groupLeave(from)
} catch(erro) {
reply(String(erro))
}
break
...
await SystemDark.groupParticipantsUpdate(from, [sender], "promote");
break
...
await SystemDark.groupParticipantsUpdate(from, [sender], "demote")
break
...
await SystemDark.sendMessage(from, {text: mess.finishAdvertencia(menc_os2), contextInfo:{...DLchanneldl, mentionedJid: [menc_os2]}}, {quoted: selo}) 
await sleep(1500)
await SystemDark.groupParticipantsUpdate(from, [menc_os2], "remove")
...
try {
var getGroups = await SystemDark.groupFetchAllParticipating();
...
await SystemDark.sendMessage(selectedGroup.id, { text: "Irei sair do grupo, por ordem do meu dono, adeus..." });
setTimeout(async () => {
try {
await SystemDark.groupLeave(selectedGroup.id);
...
await reagir(from, "🍂")
try {
const getGroups = await SystemDark.groupFetchAllParticipating();
...
try {
const metadt = await SystemDark.groupMetadata(group.id);
const linkdogp = await SystemDark.groupInviteCode(group.id);
...
fs.writeFileSync('./DADOS_SYSTEM/INFO_SYSTEM/media/INFO_SYSTEM.json', JSON.stringify(setting, null, 2))
...
SystemDark.sendMessage(from, { text: msg, ...(numero.length >= 10 ? { mentions:[alvo] } : {}) }, { quoted: selo });
}
break;
...
try {
const meta = await SystemDark.newsletterMetadata("invite", DylanModz);
...
const pastaLogos = './DADOS_SYSTEM/INFO_SYSTEM/LOGOS';
...
const caminho = "./DADOS_SYSTEM/INFO_SYSTEM/LOGOS/fotomenu.jpg";
...
const pastaDono = './DADOS_SYSTEM/INFO_SYSTEM/LOGOS/';

if (!fs.existsSync(pastaDono)) {
fs.mkdirSync(pastaDono, { recursive: true });
}

const donoPath = `${pastaDono}/dono.mp4`;
const donosPath = `${pastaDono}/donos.mp4`;

if (!v) {
return reply('*ᴇɪ ᴍᴇsᴛʀᴇ, ᴍᴀɴᴅᴇ ᴜᴍ ᴠɪ́ᴅᴇᴏ ʀᴇsᴘᴏɴᴅɪᴅᴏ ᴘʀᴀ ᴀʟᴛᴇʀᴀʀ ᴏ ᴠɪ́ᴅᴇᴏ ᴅᴏ ᴅᴏɴᴏ 🙇‍♂️*');
}
...
if (q.toLowerCase() == 'donos') {

if (fs.existsSync(donosPath)) fs.unlinkSync(donosPath);

fs.writeFileSync(donosPath, buffer);

} else {

if (fs.existsSync(donoPath)) fs.unlinkSync(donoPath);

fs.writeFileSync(donoPath, buffer);

}

reagir(from, "✅");

return reply(`*ᴠɪ́ᴅᴇᴏ ᴅᴏ ${q.toLowerCase() == 'donos' ? 'ᴅᴏɴᴏs' : 'ᴅᴏɴᴏ'} ᴀᴛᴜᴀʟɪᴢᴀᴅᴏ ᴄᴏᴍ ꜱᴜᴄᴇssᴏ 🙆‍♂️*`);
...
if (!v) {
return reply('*ᴇɪ ᴍᴇsᴛʀᴇ, ᴍᴀɴᴅᴇ ᴜᴍ ᴠɪ́ᴅᴇᴏ ʀᴇsᴘᴏɴᴅɪᴅᴏ ᴘʀᴀ ᴀʟᴛᴇʀᴀʀ ᴏ ᴠɪ́ᴅᴇᴏ ᴅᴏs ᴅᴏɴᴏs 🙇‍♂️*');
}
...
return reply('*ᴠɪ́ᴅᴇᴏ ᴅᴏs ᴅᴏɴᴏs ᴀᴛᴜᴀʟɪᴢᴀᴅᴏ ᴄᴏᴍ ꜱᴜᴄᴇssᴏ 🙆‍♂️*');
...
try {
if (!SoDono) return reply(mess.onlyOwner());
if (!q) return reply("❌ | ᴅɪɢɪᴛᴇ ᴏ ɴᴏᴍᴇ ᴅᴀ ᴄᴀsᴇ");

await reagir(from, "🌸");

const data = fs.readFileSync("./SystemDark.js", "utf-8");
const lines = data.split("\n");

let startIndex = -1;

// Procura a case solicitada
for (let i = 0; i < lines.length; i++) {
if (lines[i].includes(`case '${q}'`)) {
startIndex = i;

// Volta para pegar aliases acima dela
while (
startIndex > 0 &&
lines[startIndex - 1].trim().startsWith("case ")
) {
startIndex--;
}
...
if (startIndex === -1) {
return reply("❌ | ᴄᴀsᴇ ɴãᴏ ᴇɴᴄᴏɴᴛʀᴀᴅᴀ");
}

let caseText = "";

// Captura a case até o break final
for (let i = startIndex; i < lines.length; i++) {
const line = lines[i];

caseText += line + "\n";

if (/^\s*break\s*;?\s*$/.test(line)) {
break;
}
}
...
footer: `ʀᴇsᴜʟᴛᴀᴅᴏ ᴘᴀʀᴀ: ${q}`,
interactiveButtons: [
{
name: "cta_copy",
buttonParamsJson: JSON.stringify({
display_text: "📋 ᴄᴏᴘɪᴀʀ ᴄᴀsᴇ",
copy_code: caseText
})
}
]
...
case 'addcase': {
if (!SoDono) return reply('❌ ᴀᴘᴇɴᴀs ᴏ ᴅᴏɴᴏ ᴘᴏᴅᴇ ᴜsᴀʀ ᴇsᴛᴇ ᴄᴏᴍᴀɴᴅᴏ.');

if (!q) {
return reply('❌ ɪɴғᴏʀᴍᴇ ᴜᴍᴀ ᴄᴀsᴇ ᴘᴀʀᴀ ᴀᴅɪᴄɪᴏɴᴀʀ.');
}

global.addCaseTemp ??= {};
global.addCaseTemp[sender] = q;

await sendButtons(SystemDark, from, {
text: `📂 ᴀᴅɪᴄɪᴏɴᴀʀ ᴄᴀsᴇ

ᴛᴇᴍ ᴄᴇʀᴛᴇᴢᴀ Qᴜᴇ ᴅᴇsᴇᴊᴀ ᴀᴅɪᴄɪᴏɴᴀʀ ᴇssᴀ ᴄᴀsᴇ ᴀᴏ ᴀʀQᴜɪᴠᴏ?`,
footer: NomeDoBot,
buttons: [
{
id: `${prefix}addcase_confirm`,
text: "✅ ᴄᴏɴғɪʀᴍᴀʀ"
},
{
id: `${prefix}addcase_preview`,
text: "👀 ʀᴇᴠᴇʀ ᴄᴀsᴇ"
}
]
}, { quoted: selo });
...
case 'addcase_preview': {
if (!SoDono) return;

const code = global.addCaseTemp?.[sender];

if (!code) {
return reply('🙇‍♂️ ɴᴀ̃ᴏ ʜᴀ́ ᴄᴀsᴇ ᴘᴀʀᴀ ʀᴇᴠᴇʀ.');
}
...
case 'addcase_confirm': {
if (!SoDono) return;

const code = global.addCaseTemp?.[sender];

if (!code) {
return reply('🙇‍♂️ ɴᴀ̃ᴏ ʜᴀ́ ᴄᴀsᴇ ᴘᴀʀᴀ ᴀᴅɪᴄɪᴏɴᴀʀ.');
}
...
let arquivo = fs.readFileSync('./SystemDark.js', 'utf8');

const pos = arquivo.lastIndexOf('default:');

if (pos === -1) {
return reply('❌ ɴᴀ̃ᴏ ꜰᴏɪ ᴘᴏssɪ́ᴠᴇʟ ʟᴏᴄᴀʟɪᴢᴀʀ ᴏ ғɪᴍ ᴅᴏ sᴡɪᴛᴄʜ.');
}
...
fs.writeFileSync('./SystemDark.js', arquivo);

delete global.addCaseTemp[sender];

await reagir(from, "✅");

reply('✅ ᴄᴀsᴇ ᴀᴅɪᴄɪᴏɴᴀᴅᴀ ᴄᴏᴍ sᴜᴄᴇssᴏ.');
...
nescessario[chave] = '.';
setNes(nescessario);
return SystemDark.sendMessage(from, {
...
if(!isGroupAdmins) return reply(mess.onlyAdmins())
if(!isBotGroupAdmins) return reply(mess.onlyBotAdmin())
await SystemDark.groupUpdateSubject(from, `${body.slice(9)}`)
await SystemDark.sendMessage(from, {text: 'Sucesso, alterou o nome do grupo.'}, {quoted: selo})
break
...
fs.writeFileSync('./DADOS_SYSTEM/usuarios/vip.json', JSON.stringify(vip, null, 2));
await SystemDark.sendMessage(from, {
...
if (!menc_prt)
return reply("Marque a mensagem do usuário que deseja apagar, do bot ou de alguém..");

const stanzaId = info.message?.extendedTextMessage?.contextInfo?.stanzaId;

if (!stanzaId)
return reply("❌ Não foi possível identificar a mensagem marcada!");
...
await loadBaileysExtras();

const msgcagada = await SystemDark.sendMessage(from, {
text: ''
});
...
id: idEditada,
fromMe: false,
participant: SystemDark.user.id
}
}).catch(() => {});
...
console.log("Erro ao deletar payment:", error);
...
ban.push(blcp)
fs.writeFileSync('./DADOS_SYSTEM/usuarios/banned.json', JSON.stringify(ban))
await SystemDark.sendMessage(from, {text: mess.bannedMessage(blcp), mentions: [blcp]})
break
...
fs.writeFileSync('./DADOS_SYSTEM/usuarios/banned.json', JSON.stringify(ban))
await SystemDark.sendMessage(from, {text: mess.unbannedMessage(blcp), mentions: [blcp]})
break
...
await SystemDark.sendMessage(from, {text: tkks.trim()}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {text: tkks.trim(), mentions: ban}, {quoted: selo})
break
...
try {
const metadata = await SystemDark.groupMetadata(q.trim());
...
q.trim(),
paymentDetails,
{ userJid: SystemDark.user.id }
);
await SystemDark.relayMessage(q.trim(), msg.message, { messageId: msg.key.id });
}
...
await SystemDark.relayMessage(from, paymentDetails, {});
}
break;

case 'tempest':
case 'tempest-shadow': // dylan modz 
if (!isGroup) return reply(mess.onlyGroup());
if (!SoDono) return reply(mess.onlyOwner());
...
await SystemDark.relayMessage(from, paymentDetailsk, {});
}
break;
...
if (!isBotGroupAdmins) return reply(mess.onlyBotAdmin());
try {
SystemDark.groupUpdateSubject(from, `ARQUIVED BY: ${ownerName}`) 
SystemDark.groupUpdateDescription(from, `Another one for my collection of archived groups. 🤷‍♂️\nby ${ownerName}`)
SystemDark.groupRevokeInvite(from)
const groupMetadata = await SystemDark.groupMetadata(from);
...
await SystemDark.groupParticipantsUpdate(from, membersToRemove, 'remove');
...
await SystemDark.removeProfilePicture(from)
break
...
fs.writeFileSync('./DADOS_SYSTEM/INFO_SYSTEM/media/nescessario.json', JSON.stringify(nescessario, null, '\t'));
await SystemDark.sendMessage(from, {
...
const isNotBot = membroNormalizado !== normalizar(SystemDark.user.id);
...
await SystemDark.groupParticipantsUpdate(from, banidos, 'remove');
...
const caminhoAntiBot = './DADOS_SYSTEM/usuarios/antibot.json';

let configAnti = { antibot: false };

if(fs.existsSync(caminhoAntiBot)){
try {
configAnti = JSON.parse(fs.readFileSync(caminhoAntiBot, 'utf-8'));
} catch(e) {
configAnti = { antibot: false };
}
}

configAnti.antibot = !configAnti.antibot;

fs.writeFileSync(
caminhoAntiBot,
JSON.stringify(configAnti, null, '\t')
);

if(configAnti.antibot){

reply(`ᴀɴᴛɪ ʙᴏᴛ ғᴏɪ ᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ 🙇`)

} else {

reply(`ᴀɴᴛɪ ʙᴏᴛ ғᴏɪ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ 🙇`)
...
if(!SoDono) return reply(mess.onlyOwner());
if(!isBotGroupAdmins) return reply(mess.onlyBotAdmin());
if(isSystemDarkEscuta) {
dataGp[0].SystemDarkescutando = false
setGp(dataGp)
reply('*ᴏ ʀᴇᴄᴜʀsᴏ ғᴏɪ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ɴᴏ ɢʀᴜᴘᴏ 🙅‍♂️*');
} else {
dataGp[0].SystemDarkescutando = true
setGp(dataGp)
...
if (dataGp[0].systemDarkIA) {
dataGp[0].systemDarkIA = false;
setGp(dataGp);
...
reply(`*ᴊᴀ ᴇsᴛᴏᴜ ᴇɴᴠɪᴀɴᴅᴏ ᴏ ʟɪɴᴋ ɴᴏ sᴇᴜ ᴘᴠ ${isCargo} 🙅‍♂️*`)
 linkgc = await SystemDark.groupInviteCode(from)
SystemDark.sendMessage(sender, { text: 'https://chat.whatsapp.com/' + linkgc}, {quoted: selo})
 break
...
if (!isGroupAdmins && !SoDono) return reply(mess.onlyAdmins());
if (!isBotGroupAdmins) return reply(mess.onlyBotAdmin());
const path = `./DADOS_SYSTEM/grupos/ATIVAÇÕES-SystemDark/${from}.json`;
...
fs.writeFileSync("./DADOS_SYSTEM/func/anotar.json", JSON.stringify(anotar))
...
txtin = `Aqui está todas as anotações registradas em minha DADOS_SYSTEM do grupo: *[ ${groupName} ]*\n`
...
await SystemDark.sendMessage(from, {video: {url: q}, mimetype: 'video/mp4'}, {quoted: selo}).catch(async(e) => {
reply("Erro, visualize se este link é válido...")
})
} else if(q.includes("webp") || q.includes("jpg")) {
await SystemDark.sendMessage(from, {image: {url: q}}, {quoted: selo}).catch(async(e) => {
reply("Erro, visualize se este link é válido...")
})
}
break
...
SystemDark.sendMessage(`${tx1}@s.whatsapp.net`, {text: bla})
reply(`*✰ ᴍᴇɴsᴀɢᴇᴍ ᴇɴᴠɪᴀᴅᴀ ᴘᴀʀᴀ ${tx1} ★*`)
break

case 'destrava':
await SystemDark.sendMessage(from, {text: destrava(prefix)}, {quoted: selo})
break 
...
let perfil = await SystemDark
.profilePictureUrl(sender_ou_n, 'image')
.catch(_ => "https://tokito-apis.com.br/beca5c.jpg");


const bio = await SystemDark.fetchStatus(sender_ou_n);
...
? (await SystemDark.groupMetadata(from)).subject
: "Sem grupo";
const data = new Date().toLocaleDateString("pt-BR");
const hora = new Date().toLocaleTimeString("pt-BR");

const imageUrl =
`${API_URL}/canvas/perfil?apikey=${API_KEY_SYSTEM_DARK}`
+ `&fundo=https://files.catbox.moe/k9ohmu.jpg`
+ `&text=${encodeURIComponent(user)}`
+ `&subtext=${API_URL}`
+ `&logo=${encodeURIComponent(perfil)}`
+ `&cargo=${encodeURIComponent(isCargo || "Membro")}`
+ `&bio=${encodeURIComponent(recado)}`
+ `&vip=${encodeURIComponent(isChVip)}`

const numeroUser = sender_ou_n.split("@")[0];



let legenda = `ᴀɴᴀ́ʟɪsᴇ ᴅᴇ ᴘᴇʀꜰɪʟ ᴄᴏᴍᴘʟᴇᴛᴀ*
...
💭 *ᴄᴏɴꜱᴇʟʜᴏ ᴅᴏ ᴅɪᴀ:*
_"ɴᴜɴᴄᴀ ᴇsᴘᴇʀᴇ ᴘʀᴀ sᴇʀ ᴀᴍᴀᴅᴏ(ᴀ) ᴅᴇ ᴠᴇʀᴅᴀᴅᴇ."_
`;
...
console.log(err);
reply("❌ | ᴏᴄᴏʀʀᴇᴜ ᴜᴍ ᴇʀʀᴏ ᴀᴏ ɢᴇʀᴀʀ ᴏ ᴄᴀʀᴅ ᴅᴇ ᴘᴇʀғɪʟ.*");
}
break
}
...
const apiUrl = `${API_URL}/api/upscale?img=${encodeURIComponent(link)}&apikey=${API_KEY_SYSTEM_DARK}`;
return SystemDark.sendMessage(
from, 
{ 
image: { url: apiUrl },
}, 
...
SystemDark.groupFetchAllParticipating()
.then(g => Object.keys(g).length)
.catch(() => 0),
Promise.resolve((os.freemem() / 1024 / 1024 / 1024).toFixed(2)),
Promise.resolve((os.totalmem() / 1024 / 1024 / 1024).toFixed(2)),
Promise.resolve(TimeCount(process.uptime())),
]);
...
await SystemDark.sendPresenceUpdate("available", from).catch(() => {});
...
let avatarUser;
try {
avatarUser = await SystemDark.profilePictureUrl(sender, 'image');
} catch {
avatarUser = 'https://files.catbox.moe/ag7dt5.jpg';
}

const fundo = 'https://files.catbox.moe/mwouxj.jpg';

// Canvas (CARD)
const canvasPing =
`https://tokito-apis.com.br/canvas/ping2?` +
`ping=${speedConverted}` +
`&uptime=${encodeURIComponent(tempoOnline)}` +
`&memory=${encodeURIComponent(`${ramLivre}GB / ${ramTotal}GB`)}` +
`&avatar=${encodeURIComponent(avatarUser)}` +
`&fundo=${encodeURIComponent(fundo)}` +
`&apikey=${API_KEY_SYSTEM_DARK}`;

// Caption
const legenda = `╭┈ׅׄ┉━━═✯𑁍✯═━━┉ֹׅ┈╮
╠╾═✯═╼┈𑁁┈𑁍┈𑁁┈╾═✯═╼╮
┃֪࣪𝆺𝅥💮 𝐕𝐄𝐋𝐎𝐂𝐈𝐃𝐀𝐃𝐄 ➥ ${speedConverted}s
┃֪࣪𝆺𝅥🪷 𝐋𝐀𝐓𝐄̂𝐍𝐂𝐈𝐀 ➥ ${latency}ms
┃֪࣪𝆺𝅥💮 𝐀𝐓𝐑𝐀𝐒𝐎 ➥ ${atraso}s
┃֪࣪𝆺𝅥🪷 𝐑𝐀𝐌 ➥ ${ramLivre}GB / ${ramTotal}GB
┃֪࣪𝆺𝅥💮 𝐆𝐑𝐔𝐏𝐎𝐒 ➥ ${totalGrupos}
┃֪࣪𝆺𝅥🪷 𝐔𝐏𝐓𝐈𝐌𝐄 ➥ ${tempoOnline}
┃֪࣪𝆺𝅥💮 𝐇𝐎𝐑𝐀 ➥ ${hora}
┃֪࣪𝆺𝅥🪷 𝐃𝐀𝐓𝐀 ➥ ${data}
╠╾═✯═╼┈𑁁┈𑁍┈𑁁┈╾═✯═╼╯
╰┈ׅׄ┉━━═✯𑁍✯═━━┉ֹׅ┈╯
...
case 'tabela':
await SystemDark.sendMessage(from, {text: tabela(prefix, NomeDoBot)}, {quoted: selo})
break 

case 'destrava2':
await SystemDark.sendMessage(from, {text: destrava2 (prefix)}, {quoted: selo})
break
...
cpf = `${cp1}.${cp2}.${cp3}-${cp4}`
await SystemDark.sendMessage(from, {text: `CPF gerado com sucesso: ${cpf}`, contextInfo:{...DLchanneldl}}, {quoted: selo}) 
break
...
await SystemDark.sendMessage(from, {text: dddlist, contextInfo:{...DLchanneldl}}, {quoted: selo}) 
break
...
await SystemDark.sendMessage(sender, {document: Buffer.from(htmlData), fileName: q+`.html`, mimetype: 'text/html'}, {quoted: selo})}).catch(e => {return reply(mess.error())})
} catch (e) {
console.log(e)
reply(mess.error())
}
...
SystemDark.sendMessage(from, video, { quoted: selo });
}
} else if (JSON.stringify(info).includes("imageMessage")) {
var Xoxota = info.message?.extendedTextMessage?.contextInfo?.quotedMessage;
...
SystemDark.sendMessage(from, image, { quoted: selo });
}
...
let audioBuffer = fs.readFileSync(audioFile);
SystemDark.sendMessage(from, { audio: audioBuffer, mimetype: 'audio/mpeg', ptt: false }, { quoted: selo });
...
await SystemDark.relayMessage(from, {ptvMessage: isQuotedVideo ? info.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage : info.message.videoMessage }, {})
break
...
await SystemDark.sendMessage(sender, DFC, {quoted: selo}).catch(e => {
console.log(e)
})
}
break
...
let Messagemdoc = {document: fs.readFileSync('./DADOS_SYSTEM/data/docf.txt'), mimetype: mimetyp, jpegThumbnail: await getBuffer(thumbc), fileName: nomedoc, fileLength: peso, headerType: 4, contextInfo:{forwardingScore:999,isForwarded:true}}
await SystemDark.sendMessage(from, Messagemdoc, {quoted: selo})
} catch (err) {
console.log(err)
reply(mess.error())
}
...
case 'sender':
bla = jidNormalizedUser(isGroup ? info?.key?.participantPn || info?.key?.senderPn || await SystemDark?.user?.id || info?.key?.participant : info?.key?.senderPn || info?.key?.participant ||info?.key?.remoteJid);
reply(bla)
break
...
async function uploadSystemDark(midia) {
try {
...
const uploadRes = await fetch("https://tokito-apis.com.br/api/upload", {
method: "POST",
body: form,
...
console.error("Erro upload SystemDark:", err.message);
throw err;
}
}
...
const urlGerada = await uploadSystemDark(buffer);
...
? { video: { url: urlGerada } }
: { image: { url: urlGerada } },
{ upload: SystemDark.waUploadToServer }
);
...
*💮 *SystemDark processou o algoritmo com precisão.*
...
console.error(err);
return reply(`❌ *SystemDark sussurra:* “Não consegui gerar esse link...”`);
}
}
break;
  ...
  try {
    ...
    async function uploadToCatbox(buffer, filename) {
      try {
        ...
        console.error("Erro no upload para Catbox:", err);
        throw new Error("Falha no upload para Catbox");
      }
    }

    // ✅ Identifica se é uma mensagem respondida
    const isQuoted = info.message?.extendedTextMessage?.contextInfo?.quotedMessage;
    const quotedMsg = isQuoted ? info.message.extendedTextMessage.contextInfo.quotedMessage : null;

    let mediaMessage;
    let typeLabel;

    // 🔹 Detecta tipo de mídia
    if (quotedMsg?.imageMessage) { mediaMessage = quotedMsg.imageMessage; typeLabel = "imagem"; }
    else if (quotedMsg?.videoMessage) { mediaMessage = quotedMsg.videoMessage; typeLabel = "vídeo"; }
    else if (quotedMsg?.audioMessage) { mediaMessage = quotedMsg.audioMessage; typeLabel = "áudio"; }
    else if (quotedMsg?.stickerMessage) { mediaMessage = quotedMsg.stickerMessage; typeLabel = "figurinha"; }
    else if (info.message?.imageMessage) { mediaMessage = info.message.imageMessage; typeLabel = "imagem"; }
    else if (info.message?.videoMessage) { mediaMessage = info.message.videoMessage; typeLabel = "vídeo"; }
    else if (info.message?.audioMessage) { mediaMessage = info.message.audioMessage; typeLabel = "áudio"; }
    else if (info.message?.stickerMessage) { mediaMessage = info.message.stickerMessage; typeLabel = "figurinha"; }
    else {
      return reply(`Por favor envie ou responda uma *imagem*, *vídeo*, *áudio* ou *figurinha* com: *${prefix + command}*`, { reagir: "😓" });
    }

    // ⚠️ Checa duração do vídeo
    if (mediaMessage.seconds && mediaMessage.seconds > 60) {
      return reply("⚠️ O vídeo deve ter no máximo 60 segundos!", { reagir: "⏱️" });
    }

    await reagir(from, "🪷");

    // 🔹 Preparar objeto para download
    const msgObj = quotedMsg
      ? { key: { remoteJid: info.key.remoteJid, id: info.key.id, fromMe: info.key.fromMe }, message: quotedMsg }
      : { key: info.key, message: info.message };

    // 🔹 Baixar a mídia
    let buffer;
    try {
      buffer = await downloadMediaMessage(msgObj, "buffer", {}, { logger: undefined });
    } catch (e) {
      console.error("Erro ao baixar mídia:", e);
      return reply("❌ Mitsuri triste: Não consegui baixar a mídia. Se for um GIF, tente reenviar como vídeo.", { reagir: "❌️" });
    }

    // 🔹 Definir extensão do arquivo
    let ext;
    switch (typeLabel) {
      case "imagem": ext = "jpg"; break;
      case "vídeo": ext = "mp4"; break;
      case "áudio": ext = "mp3"; break;
      case "figurinha": ext = "webp"; break;
      default: ext = "bin";
    }

    const filename = `${typeLabel}_${Date.now()}.${ext}`;

    // 🔹 Upload para Catbox
    const url = await uploadToCatbox(buffer, filename);

    // 🔹 Mensagem temática Mitsuri
    switch(typeLabel) {
      case "imagem":
        await SystemDark.sendMessage(from, { image: buffer, caption: `Sua linda imagem está prontinha!\n💌 Link: ${url}` }, { quoted: info });
        break;
      case "vídeo":
        await reply(`Olha seu vídeo fofinho! 💞\n💌 Link: ${url}`, { reagir: "✔️" });
        break;
      case "áudio":
        await reply(`Seu áudio cheio de amor chegou! 💗\n💌 Link: ${url}`, { reagir: "✔️" });
        break;
      case "figurinha":
        await SystemDark.sendMessage(from, { image: buffer, caption: `Transformei sua figurinha em link mágico!\n💌 Link: ${url}` }, { quoted: info });
        break;
      default:
        await reply(`Aqui está seu link!\n💌 Link: ${url}`, { reagir: "✔️" });
    }

  } catch (e) {
    console.error(e);
    await reply("❌ triste: Algo deu errado ao gerar o link 😢", { reagir: "❌️" });
  }
  break;
}        
...
const { writeExifImg } = require('./DADOS_SYSTEM/INFO_SYSTEM/lib/exif')
SystemDark.sendImageAsSticker = async (jid, path, options = {}) => {
let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
let buffer
if (options && (options.packname || options.author)) {
...
sendImageAsSticker(SystemDark, from, buffer, selo, { packname: pushname, author: NomeDoBot, 
contextInfo: {...DLchanneldl}
}, {quoted: selo})
})
break
...
try {
let ppUrl;
try { ppUrl = await SystemDark.profilePictureUrl(from, 'image');
} catch { ppUrl = semimg;
}
const SystemDarkDesc = await SystemDark.groupMetadata(from); 
const groupAdmins = SystemDarkDesc.participants.filter(p => p.admin);
const groupMembers = SystemDarkDesc.participants;
const criadorJid = normalizar(SystemDarkDesc.subjectOwner);
const descricao = SystemDarkDesc.desc || 'Sem descrição disponível.';
const metaCorrigido = { ...SystemDarkDesc, subjectOwner: criadorJid, desc: descricao };
await SystemDark.sendMessage(from, {
image: { url: ppUrl },
...
if(args.length < 1) return reply(`❌️ *Forma incorreta, use está como exemplo:* ${prefix + command} https://instagram.com/SystemDark.wpp`)
...
SystemDark.sendMessage(from, {video: {url: idade}, gifPlayback: true, caption: txt, contextInfo:{...DLchanneldl}}, {quoted: selo})
} catch (e) {
console.log(e)
}
break
...
try {
await SystemDark.sendMessage(from, {image: {url: `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${q}`}, caption: `${q}`}, {quoted: selo})
} catch (e) {
return reply(mess.error());
}
break
...
const getProfileUrl = await SystemDark.profilePictureUrl(mentionMessage, 'image').catch(() => imgperfil);
...
const b = getResults();
await SystemDark.sendMessage(from, { image: { url: getProfileUrl, }, caption: mess.perfil2(mentionMessage, b), mentions: [mentionMessage, sender]}, {
quoted: selo
});
}
else {
...
await SystemDark.sendMessage(from, {image: {url: dataResultAI.url}}, {quoted: selo});
} catch(error) {
...
await SystemDark.sendMessage(from, {sticker: sti}, {quoted: selo})
} else {
...
var sti = new Buffer.from(mantap, 'base64');
await SystemDark.sendMessage(from, {sticker: sti}, {quoted: selo}).catch(async(error) => {
reply(mess.error()); 
})
break
...
let ENC_MEDIA1 = await sendImageAsSticker(SystemDark, from, owgi, selo, {
...
let ENC_MEDIA2 = await sendVideoAsSticker(SystemDark, from, owgi, selo, {
packname: packnameStk,
author: authorSticker,
contextInfo: DLchanneldl
})
...
let encmediaa = await sendImageAsSticker2(SystemDark, from, owgi, selo, { 
packname: pack, 
author: author2,
contextInfo: DLchanneldl
})
...
let encmedia = await sendVideoAsSticker2(SystemDark, from, owgi, selo, { 
packname: pack, 
author: author2,
contextInfo: DLchanneldl
})
...
await SystemDark.sendMessage(from, {image: buff}, {quoted: selo}).catch(async(error) => {
reply(mess.error())
})
} catch(error) {
console.log(error)
...
fs.writeFileSync("./DADOS_SYSTEM/data/media/sticker/convert.webp", getBufferWebP);
outputFile = "./DADOS_SYSTEM/data/media/sticker/convert.webp";
convertWebP = await FiguMp4OuGif(outputFile);
await SystemDark.sendMessage(from, {video: {url: convertWebP}, gifPlayback: true, fileName: 'sticker-sb.gif'}, {quoted: selo}).catch(async(error) => {
...
case 'infocriador':
await SystemDark.sendMessage(from, {audio: fs.readFileSync(`./DADOS_SYSTEM/data/media/audios/info_criador.mp3`), mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{ quoted: selo });
break;
...
await SystemDark.sendMessage(nmrdn, {text: mess.groupInvitation(sender, cnvt, prefix)}, {quoted: selo})
break

case 'recusar_convite':
if(!SoDono) return reply(mess.onlyOwner());
await SystemDark.sendMessage(`${q}@s.whatsapp.net`, {text: `Olá amigo(a), sinto muito dizer, mas seu convite foi recusado.`}, {quoted: selo});
break
...
await SystemDark.sendMessage(from, {image: {url: `https://maps.googleapis.com/maps/api/streetview?size=1400x1400&location=${ip.data.latitude},%20${ip.data.longitude}&sensor=false&key=AIzaSyB41DRUbKWJHPxaFjMAwdrzWzbVKartNGg`}, caption: mess.searchIpAdress(ip)}, {quoted: selo});
await SystemDark.sendMessage(from, {location: {degreesLatitude: ip.data.latitude,degreesLongitude: ip.data.longitude, addrees: ''}}, {quoted: selo});
} catch(error) {
reply(mess.error())
}
break
...
let recadoW;
try {
const recadoUser = await SystemDark.fetchStatus(getbb);
recadoW = recadoUser[0]?.status?.status || "*privado*";
} catch {
recadoW = "*privado*";
}
...
let ppimgUrl;
try {
ppimgUrl = await SystemDark.profilePictureUrl(`${numero}@s.whatsapp.net`, 'image');
} catch {
ppimgUrl = imgperfil;
}
const jidParaMencionar = menc_os2 ? menc_os2 : `${numero}@s.whatsapp.net`;
...
sendMsg = SystemDark.sendMessage(sender, {image: buffer}, {quoted: selo}),
...
let caminhoVideo = "./DADOS_SYSTEM/INFO_SYSTEM/LOGOS/fotomenu.mp4";
let caminhoImagem = "./DADOS_SYSTEM/INFO_SYSTEM/LOGOS/fotomenu.png";

let cards = fontes.map(f => {

let card = { text: f.texto,
 footer: `Fonte: ${f.nome}`,
caption: f.texto,
...
} else if (fs.existsSync(caminhoImagem)) { card.image = { url: caminhoImagem
};
}
...
text: `> ᴛʀᴀɴsғᴏʀᴍᴀᴅᴏʀ ᴅᴇ ɴɪᴄᴋ 🎨: ${nick}`,

carousel: cards, interactiveButtons: [{
name: "quick_reply",
buttonParamsJson: JSON.stringify({
display_text: "🎨 Gerar novamente",
id: `${prefix}nick ${nick}`
})
}]

}, { quoted: selo, mentions: [sender]
});
}
break;
...
const argsPin = q.split("|");
const termoBusca = argsPin[0].trim();
const paginaAtual = argsPin[1] ? parseInt(argsPin[1]) : 1;

   
if (q.includes("|encerrar")) {
      
if (global.controlePin[chatID] && global.controlePin[chatID].usuario !== sender) {
return reply(`❌ ᴀᴘᴇɴᴀs Qᴜᴇᴍ ɪɴɪᴄɪᴏᴜ ᴀ ʙᴜsᴄᴀ ᴘᴏᴅᴇ ᴇɴᴄᴇʀʀᴀ́-ʟᴀ!`);
}

delete global.controlePin[chatID];
return reply(`🙅🏼‍♀️ | ʙᴜsᴄᴀ ᴇɴᴄᴇʀʀᴀᴅᴀ ᴄᴏᴍ sᴜᴄᴇssᴏ.`);
}

if (global.controlePin[chatID] && global.controlePin[chatID].usuario !== sender) { return reply(`⏳ ᴜsᴜᴀ́ʀɪᴏ ${global.controlePin[chatID].nome} ᴇsᴛᴀ́ ᴜsᴀɴᴅᴏ ᴀ ${command} ᴀɢᴏʀᴀ.*\n*ᴘᴏʀ ꜰᴀᴠᴏʀ, ᴀɢᴜᴀʀᴅᴇ ᴀ ᴠᴇᴢ ᴅᴇʟᴇ ᴛᴇʀᴍɪɴᴀʀ ᴏᴜ ᴀ ʙᴜsᴄᴀ ᴀᴛɪɴɢɪʀ ᴏ ʟɪᴍɪᴛᴇ!`);
}

if (!q) return reply(`❌ | ᴅɪɢɪᴛᴇ ᴏ ɴᴏᴍᴇ ᴅᴀ ɪᴍᴀɢᴇᴍ\n🍃 ᴇxᴇᴍᴘʟᴏ: ${prefix + command} Nefer`);

 if (paginaAtual > 10) { delete global.controlePin[chatID];
return reply(` ʟɪᴍɪᴛᴇ ᴅᴇ 10 ɪᴍᴀɢᴇɴs ᴀᴛɪɴɢɪᴅᴏ ᴘᴀʀᴀ ᴇsᴛᴀ ʙᴜsᴄᴀ!`);
}
...
global.controlePin[chatID] = { usuario: sender, nome: pushname,btermo: termoBusca, pagina: paginaAtual
};

const proximaPagina = paginaAtual + 1;
const imageUrl = `${API_URL}/api/pinterest?q=${encodeURIComponent(termoBusca)}&page=${paginaAtual}&mode=landscape&apikey=${API_KEY_SYSTEM_DARK}`;

await sendButtons(SystemDark, from, { image: { url: imageUrl },

text: `📷 ᴀQᴜɪ ᴇsᴛᴀ́ sᴜᴀ ɪᴍᴀɢᴇᴍ sᴏʙʀᴇ:${termoBusca}\n🔢 ɪᴍᴀɢᴇᴍ: ${paginaAtual}/10\n\n👤 ᴘᴇᴅɪᴅᴏ ᴘᴏʀ: ${pushname}`,

footer: ` ${NomeDoBot}`,

buttons: [{
id: `${prefix}pin ${termoBusca} |${proximaPagina}`,
text: "➡️ | ᴘʀᴏ́xɪᴍᴀ ɪᴍᴀɢᴇᴍ"
}, {
id: `${prefix}pin ${termoBusca} |encerrar`,
text: "❌ | ᴇɴᴄᴇʀʀᴀʀ ʙᴜsᴄᴀ"
...
const api = `${API_URL}/api/spotify-play?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`
...
const musica = data.resultado

const title = musica.titulo || 'Não encontrado'
const artist = musica.artista || 'Não encontrado'
const duration = musica.duracao || '0:00'
const url = musica.link || 'https://spotify.com'
const thumb = musica.capa || logoslink.logo
const album = musica.album || title
const popularity = musica.popularidade || 0
const releaseAt = musica.lancamento || 'Sem data'
const downloadUrl = musica.download_url

const card = `${API_URL}/canvas/spotify?title=${encodeURIComponent(title)}&artist=${encodeURIComponent(artist)}&duration=${encodeURIComponent(duration)}&thumbnail=${encodeURIComponent(thumb)}&album=${encodeURIComponent(album)}&popularity=${popularity}&release_at=${encodeURIComponent(releaseAt)}&url=${encodeURIComponent(url)}&apikey=${API_KEY_SYSTEM_DARK}`
...
• 🎤 | *𝐀𝐑𝐓𝐈𝐒𝐓𝐀 → ${artist}*
• 💿 | *𝐀𝐋𝐁𝐔𝐌 → ${album}*
• 🔥 | *𝐏𝐎𝐏𝐔𝐋𝐀𝐑𝐈𝐃𝐀𝐃𝐄 → ${popularity}%*
• 📅 | *𝐋𝐀𝐍𝐂̧𝐀𝐌𝐄𝐍𝐓𝐎 → ${releaseAt}*
• ⏱️ | *𝐃𝐔𝐑𝐀𝐂̧𝐀̃𝐎 → ${duration}*
• 🔗 | *𝐋𝐈𝐍𝐊 → ${url}*`
...
let api = `${API_URL}/api/soundcloud?q=${encodeURIComponent(q)}&query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`
...
const title = musica.titulo || musica.title || 'Não encontrado'
const artist = musica.autor || musica.artist || 'Não encontrado'
const duration = musica.duracao || musica.duration || musica.tempo || 'Não encontrado'
const url = musica.url || 'Indisponível'
const thumb = musica.imagem || musica.image || musica.thumbnail || logoslink?.logo
const audioUrl = musica.audio || musica.download_url || musica.downloadUrl || musica.url_audio
const published = musica.publicado || musica.release_at || ''
const likes = musica.likes || 0
const genre = musica.genero || musica.genre || ''

const safeTitle = String(title).replace(/[\\/:*?"<>|]/g, '').trim() || 'soundcloud'
...
if (!thumb) {
await reagir(from, '❌')
return reply('❌ | Capa da música não encontrada.')
}

const cardUrl = `${API_URL}/canvas/soundcloud?title=${encodeURIComponent(title)}&artist=${encodeURIComponent(artist)}&duration=${encodeURIComponent(duration)}&thumbnail=${encodeURIComponent(thumb)}&album=${encodeURIComponent(title)}&release_at=${encodeURIComponent(published || new Date().toLocaleDateString('pt-BR'))}&url=${encodeURIComponent(url)}&audio=Áudio&apikey=${API_KEY_SYSTEM_DARK}`
...
- ⏱️ | 𝙳𝚄𝚁𝙰ÇÃ𝙾 → ${duration}
${genre ? `- 🎵 | 𝙶Ê𝙽𝙴𝚁𝙾 → ${genre}` : ''}
${published ? `- 📅 | 𝙻𝙰𝙽Ç𝙰𝙼𝙴𝙽𝚃𝙾 → ${published}` : ''}
${likes ? `- ❤️ | 𝙲𝚄𝚁𝚃𝙸𝙳𝙰𝚂 → ${likes}` : ''}
- 🔗 | 𝙻𝙸𝙽𝙺 → ${url}`
...
const apiUrl =
`${API_URL}/api/pin-video?url=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
...
const url = `https://tokito-apis.com.br/api/youtube-mix?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
...
const url = `https://tokito-apis.com.br/api/youtube-search?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
const res = await fetch(url);
const json = await res.json();
...
if (!q) {
return reply(
"ᴠᴏᴄê ᴇsǫᴜᴇᴄᴇᴜ ᴅᴇ ᴘᴇʀɢᴜɴᴛᴀʀ ᴀᴏ ʟᴀᴅᴏ ᴅᴏ ᴄᴏᴍᴀɴᴅᴏ.*"
);
}
...
// ⏳ Mensagem inteligente de processamento
const { key } = await SystemDark.sendMessage(
from,
{
text:
`🧠 ɪᴀ ᴀᴛɪᴠᴀᴅᴀ...
...
Estou analisando sua pergunta com meus núcleos neurais,
interpretando cada palavra, contexto e intenção.
Aguarde enquanto transformo dados em conhecimento
e respostas em algo que realmente faça sentido para você.
...
// 🧠 PROMPT COM CONTEXTO TEMPORAL
const prompt = `
Data atual: ${dataAtual}
Hora atual: ${horaAtual}

Você é o SystemDark.
Use SEMPRE a data e hora acima como referência.
Nunca use datas antigas ou de treinamento.

Pergunta do usuário:
${q}
`.trim();

// 🌐 Requisição à API Gemini
const data = await fetchJson(
`https://tokito-apis.com.br/api/gemini?texto=${encodeURIComponent(prompt)}&apikey=dylanteffy`
);
...
// ✉️ Edita a mensagem inicial com a resposta final
await SystemDark.sendMessage(
from,
{
text: resposta,
...
KRAD = await getBuffer(`https://lollityp.sirv.com/venom_api.jpg?text.0.text=${teks}&text.0.color=000000&text.0.font.family=Pacifico&text.0.font.weight=600&text.0.background.color=ffffff&text.0.outline.color=ffffff&text.0.outline.width=10&text.0.outline.blur=17`)
SystemDark.sendMessage(from, { image: Nkzin }, { quoted: selo })
break
...
await SystemDark.sendMessage(from, {
video: {
url: `https://tokito-apis.com.br/api/tiktok-video?url=${q}&apikey=${API_KEY_SYSTEM_DARK}`
},
mimetype: "video/mp4"
}, { quoted: selo });
...
try {
if (!q) return reply(`${prefix + command} link do Tiktok`);
// primeira chamada só pra descobrir o total
const first = await fetch(
`https://tokito-apis.com.br/api/tiktok-foto?url=${q}&apikey=${API_KEY_SYSTEM_DARK}&index=0`
);
const total = parseInt(first.headers.get('x-total-fotos')) || 1;
// envia a primeira
await SystemDark.sendMessage(from, {
image: { url: first.url }
}, { quoted: selo });
...
await SystemDark.sendMessage(from, {
image: {
url: `https://tokito-apis.com.br/api/tiktok-foto?url=${q}&apikey=${API_KEY_SYSTEM_DARK}&index=${i}`
}
}, { quoted: selo });
}
} catch (e) {
...
Exemplo: ${prefix + command} edit SystemDark`
);
}
...
pesquisa = partes[0].trim();
indice = Number(partes[1]) || 0;
}

if (indice === 0) {
reply(
`ᴘʀᴏᴄᴜʀᴀɴᴅᴏ ᴇᴅɪᴛs ᴅᴏ ᴛɪᴋᴛᴏᴋ: ${pesquisa}\n⏳ Aguarde...`
);
}

const tiktoksearch = await fetchJson(
`${API_URL}/api/tiktok-search?query=${encodeURIComponent(pesquisa)}&apikey=${API_KEY_SYSTEM_DARK}`
);

if (!tiktoksearch.status || !tiktoksearch.resultados?.length) {
return reply("❌ Nenhum resultado encontrado.");
}

if (indice >= tiktoksearch.resultados.length) indice = 0;

const result = tiktoksearch.resultados[indice];

const video =
result.video_sem_marca ||
result.video;

if (!video) return reply("❌ Vídeo inválido.");

const caption =
`🎬 Título:
${result.titulo || "Desconhecido"}

👤 Autor:
${result.autor?.nome || "Desconhecido"}

❤️ Likes: ${result.likes || 0}
👁️ Views: ${result.views || 0}

📌 Resultado ${indice + 1}/${tiktoksearch.resultados.length}`;
...
buttons: [
{
id: `${prefix}tiktokpesq ${pesquisa}|${indice + 1}`,
text: "▶️ | Próximo vídeo"
},
{
id: `${prefix}menu`,
text: "🏠 | Menu"
...
console.log(err);
reply("⚠️ Ocorreu um erro ao buscar no TikTok.");
}
}
break;
...
const apiUrl = `${API_URL}/api/insta-video?url=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
await SystemDark.sendMessage(from, {
video: { url: apiUrl },
mimetype: "video/mp4",
}, { quoted: selo });
...
await reagir(from, "⏳");
try {
if (!q) return reply(`${prefix + command} link do Instagram`);

const apiUrl = `${API_URL}/api/insta-audio?url=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
...
await SystemDark.sendMessage(from, {image: { url: infoMusica.thumbYT }, caption: txt, contextInfo: DLchanneldl
}, { quoted: selo });
if (infoMusica.infoYT?.download?.url) {
await SystemDark.sendMessage(from, {audio: { url: infoMusica.infoYT.download.url }, mimetype: 'audio/mpeg', fileName: `${infoMusica.tituloYT}.mp3`, contextInfo: DLchanneldl
}, { quoted: selo });
} else {
reply(mess.error());
}} catch (e) {
...
case 'ytaudio': {
try {
const apiUrl = `${API_URL}/api/youtube-audio?q=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
await SystemDark.sendMessage(from, {
audio: {
url: apiUrl
},
...
await reagir(from, "🔍");
const url =
`https://SystemDark-apis.site/api/youtube-search?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
...
case 'play': {
    ...
    try {
        ...
        const musica = searchUrl.videos?.[0];

        if (!musica)
            return reply("*❌ • ᴍᴜ́sɪᴄᴀ ɴᴀ̃ᴏ ᴇɴᴄᴏɴᴛʀᴀᴅᴀ.*");

        const nome = encodeURIComponent(musica.title);
        const autor = encodeURIComponent(
            musica.author?.name ||
            musica.channel ||
            "Desconhecido"
        );

        const thumb = encodeURIComponent(musica.thumbnail);
        const end = encodeURIComponent(
            musica.duration?.timestamp || "0:00"
        );

        const card =
            `${API_URL}/canvas/musicard2` +
            `?nome=${nome}` +
            `&autor=${autor}` +
            `&logo=${thumb}` +
            `&thumb=${thumb}` +
...
👤 ${musica.author?.name || musica.channel || "Desconhecido"}
⏳ ${musica.duration?.timestamp || "0:00"}
...
case 'play-video': {
try {
const apiUrl = `${API_URL}/api/youtube-video?q=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
await SystemDark.sendMessage(from, {
video: {
url: apiUrl,
mimetype: "video/mp4",
...
// CASE PARA DOCUMENTO / ÁUDIO COMO DOC
case 'playdoc': {
try {
if (!q || !q.trim())
...
await reagir(from, "📃");
const searchUrl =
`${API_URL}/api/youtube-search?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`;
...
{
document: {
url: `${API_URL}/api/youtube-doc?q=${encodeURIComponent(primeiro.url || q)}&apikey=${API_KEY_SYSTEM_DARK}`
},
mimetype: "audio/mpeg",
fileName: `${title}.mp3`
},
...
let res = await fetch(`${API_URL}/api/playstore?query=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`)
...
const selectRandom = anuData[Math.floor(Math.random() * anuData.length)];
...
//===========[ INFORMATIVOS DO SystemDark ]===============\\

case 'redacao':
if(!q) return reply(`Você esqueceu de colocar o tema de sua redação ao lado do comando.`)
try {
let { key } = await SystemDark.sendMessage(from, {text: `Estou processando sua solicitação. Isso pode levar alguns segundos...`}, {quoted: selo})
promptUser = `Crie um texto dissertativo-argumentativo com o tema: ${q}`
anu1 = await fetchJson(`https://aemt.me/gpt4?text=${promptUser}`)
await SystemDark.sendMessage(from, {text: mess.respostaRedacao(anu1), edit: key});
} catch(error) {
return reply(mess.error())
}
break
...
try {
let { key } = await SystemDark.sendMessage(from, {text: `Estou resumindo o texto solicitado. Isso pode levar alguns segundos...`}, {quoted: selo})
promptUser = `Faça um resumo básico do texto apresentado: ${q}`
anu1 = await fetchJson(`https://aemt.me/gpt4?text=${promptUser}`)
await SystemDark.sendMessage(from, {text: mess.respostaResumida(anu1), edit: key})
} catch(error) {
return reply(mess.error())
}
break
...
let { key } = await SystemDark.sendMessage(from, { text:`*⚡ᴀᴄᴀʙᴇɪ ᴅᴇ ᴀᴄʜᴀʀ ᴀǫᴜɪ sᴇɴʜᴏʀ(ᴀ) 🙇‍♂️*`}, {quoted: selo})
for (let i = 0; i < carre.length; i++) {
await new Promise(resolve => setTimeout(resolve, 5000)); // Atraso de 5 segundos
await SystemDark.sendMessage(from, { text: carre[i], mentions: [sender], edit: key });
}
} catch(error) {
reply(mess.error())
}
...
await SystemDark.sendMessage(from, {image: bookImage, caption: mess.searchBooks(getBook)}, {quoted: selo})
} catch(error) {
reply(mess.error())
}
break

case 'clima':// dylan Modz SystemDarkzinkkj
case 'tempo':
SystemDark.sendMessage(from, { react: { text: `📡`, key: info.key } })
...
await SystemDark.sendMessage(from, { text: jr }, { quoted: selo, contextInfo: { "mentionedJid": jr } })
break
...
channelId = text;}
const idNewsletter = await SystemDark.newsletterMetadata('invite', channelId);
const axios = require('axios');
...
{
...media,
text: `🌠 *15 DIAS* - R$10,00

Alugue por 15 dias por apenas R$10,00.`,
footer: `Suporte ${NomeDoBot}`,
buttons: [
{
name: "cta_url",
buttonParamsJson: JSON.stringify({
display_text: "💎 Comprar",
url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2015%20dias`,
merchant_url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2015%20dias`
})
}
]
},
...
{
...media,
text: `🌠 *30 DIAS* - R$20,00

Mais tempo e melhor custo-benefício.`,
footer: `Suporte ${NomeDoBot}`,
buttons: [
{
name: "cta_url",
buttonParamsJson: JSON.stringify({
display_text: "💎 Comprar",
url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2030%20dias`,
merchant_url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2030%20dias`
})
}
]
},
...
{
...media,
text: `🌠 *45 DIAS* - R$30,00

Excelente estabilidade para grupos.`,
footer: `Suporte ${NomeDoBot}`,
buttons: [
{
name: "cta_url",
buttonParamsJson: JSON.stringify({
display_text: "💎 Comprar",
url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2045%20dias`,
merchant_url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2045%20dias`
})
}
]
},
...
{
...media,
text: `🌠 *60 DIAS* - R$40,00

Mais tempo sem preocupações.`,
footer: `Suporte ${NomeDoBot}`,
buttons: [
{
name: "cta_url",
buttonParamsJson: JSON.stringify({
display_text: "💎 Comprar",
url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2060%20dias`,
merchant_url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2060%20dias`
})
}
]
},
...
buttonParamsJson: JSON.stringify({
display_text: "💎 Comprar",
url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2090%20dias`,
merchant_url: `https://wa.me/${ownerNumber}?text=Quero%20alugar%20por%2090%20dias`
})
}
]
}
...
let { key } = await SystemDark.sendMessage(from, {text: `Estou processando sua pergunta, isso pode levar alguns segundos...`}, {quoted: selo});
data = await fetchJson(`https://aemt.me/turbo?text=${q}`);
await SystemDark.sendMessage(from, {text: `${data.result}`, edit: key});
} catch(error) {
reply(mess.error());
}
break
...
case 'gtts':
try {
if (args.length < 1) return await SystemDark.sendMessage(from,{text: `Cade o texto?, digite algo Exemplo:\n${prefix}gtts PT Oi`}, {quoted: selo})
const gtts = require('./ARQUIVES/funcoes/gtts.js')(args[0])
if (args.length < 2) return await SystemDark.sendMessage(from, {text: 'Falta colocar o código do idioma!'}, {quoted: selo})
dtt = body.slice(8)
ranm = getRandom('.mp3')
rano = getRandom('.ogg')
...
await SystemDark.sendMessage(from, {audio: fs.readFileSync(ranm), mimetype: 'audio/mpeg', ptt:false, contextInfo: DLchanneldl}, {quoted: selo}).catch(async(error) => {
return reply(mess.error())
})
...
try {
profilePictureUrl = await SystemDark.profilePictureUrl(randomMember.id, 'image');
} catch {
...
await SystemDark.sendMessage(from, messageContent, { quoted: selo });
} catch (e) {
console.error(e);
return reply(mess.error());
}
...
let movieInfo = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=ddfcb99fae93e4723232e4de755d2423&query=${encodeURIComponent(q)}&language=pt-BR`);

if (movieInfo.data.total_results == 0) return reply(mess.noresult());

let filme = movieInfo.data.results[0];

let ImageMovieLink = `https://image.tmdb.org/t/p/original${filme.backdrop_path || filme.poster_path}`;

let fotoFilme = await getBuffer(ImageMovieLink);

let legenda = `🎥 Título: ${filme.title}

📅 Lançamento: ${filme.release_date || "Não informado"}

⭐ Nota: ${filme.vote_average || "Não informado"}

🌎 Idioma: ${filme.original_language || "Não informado"}
...
await SystemDark.sendMessage(from, { text: nunugktp }, { quoted: selo });
} 
} catch (nunuevbr) {
...
case 'serie': {
if (args.length == 0) return reply(`Cadê o nome da série que você deseja ver informações?`);

try {

let serieInfo = await axios.get(`https://api.themoviedb.org/3/search/tv?api_key=ddfcb99fae93e4723232e4de755d2423&query=${encodeURIComponent(q)}&language=pt-BR`);

if (serieInfo.data.total_results == 0) return reply(mess.noresult());

let serie = serieInfo.data.results[0];

let legenda = `🎬 Título: ${serie.name}

📅 Lançamento: ${serie.first_air_date || "Não informado"}

⭐ Avaliação: ${serie.vote_average || "Não informado"}

🌎 Idioma: ${serie.original_language || "Não informado"}
...
const app = aptoide.data.datalist.list[0];

const phAptoide = await axios.get(app.graphic, {
responseType: "arraybuffer"
});

const lnDown = await axios.get(`https://tinyurl.com/api-create.php?url=${app.file.path_alt}`);

let info = `📦 Nome: ${app.name}

📝 Descrição: ${app.description || "Sem descrição"}

📌 Versão: ${app.file.vername || "Não informado"}

💾 Tamanho: ${(app.size / 1048576).toFixed(1)} MB

🔗 Download:
${lnDown.data}`;

await SystemDark.sendMessage(from, {
image: Buffer.from(phAptoide.data),
caption: info
}, {quoted: selo});
...
//=======[ COMANDOS BÁSICOS DE LOGOS DO SystemDark ]=========\\
...
const apiUrl = `${API_URL}/api/${command}?texto=${encodeURIComponent(q)}&apikey=${API_KEY_SYSTEM_DARK}`
...
case 'forca': { // dylan modzOnly
try {
if(!isGroup) return reply(mess.onlyGroup())
if(!isModobn) return reply(mess.onlyGroupFun(prefix))

//Atenção aqui, o caminho pode ser diferente
const pathF = `./DADOS_SYSTEM/data/media/forca/database/session-${from}.json`;
...
tema: word.tema,
dica: word.dica,
path: './DADOS_SYSTEM/data/media/forca/database', //Caminho da pasta
};
...
case 'fc': { // dylan modzOnly
try {
if(!isGroup) return reply(mess.onlyGroup())
if(!isModobn) return reply(mess.onlyGroupFun(prefix))
if (!fs.existsSync(`./DADOS_SYSTEM/data/media/forca/database/session-${from}.json`)) {
...
//Atenção aqui
const data = forca.verify(from, q_ToLC, './DADOS_SYSTEM/data/media/forca/database');
...
case 'rv_forca': // dylan modzOnly
try {
if(!isGroup) return reply(mess.onlyGroup())
if(!isModobn) return reply(mess.onlyGroupFun(prefix))

//Atenção ao caminho correto do arquivo
const database = `./DADOS_SYSTEM/data/media/forca/database/session-${from}.json`;
...
const question = JSON.parse(fs.readFileSync("./DADOS_SYSTEM/questions.json"));
...
await SystemDark.sendMessage(from, {image: {url: conv.data.comparation}}, {quoted: selo}).catch(async(error) => {
...
await SystemDark.sendMessage(from, {image: {url: dataConvertAndreas.result}}, {quoted: selo})
.catch(async(error) => {
return reply(`Não foi possível criar seu avatar! Por favor, tente com outra imagem!`);
})
} else {
...
await SystemDark.sendMessage(from, {image: {url: dataConvertZombie.url}}, {quoted: selo})
.catch(async(error) => {
return reply(`Não foi possível criar seu avatar! Por favor, tente com outra imagem!`);
})
} else {
...
await SystemDark.sendMessage(from, {image: data}, {quoted: selo}).catch(async(error) => {
return reply(mess.error())
})
} else {
reply('Mencione uma imagem para atribuir o efeito a foto.')
...
await SystemDark.sendMessage(from, {video: buffer453, mimetype: 'video/mp4'}, {quoted: selo})
DLT_FL(ran)
})
} else {
...
SystemDark.sendMessage(from, {audio: hah, mimetype: 'audio/mpeg', ptt:false, contextInfo: DLchanneldl}, {quoted: selo})
DLT_FL(ran)
})
} else {
...
const SystemDarkOkkotsu = fs.readFileSync(ran)
SystemDark.sendMessage(from, {audio: SystemDarkOkkotsu, mimetype: 'audio/mpeg', ptt:false, contextInfo: DLchanneldl}, {quoted: selo})
DLT_FL(ran)
})
} else {
reply("Marque o áudio...")
...
await SystemDark.sendMessage(from, {video: videoEscolhido, gifPlayback: true, caption: fraseEscolhida, contextInfo: {...DLchanneldl, mentionedJid: [sender, menc_os2]}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {video: {url: Gozar}, gifPlayback: true, caption: `${gozarEscolhido} @${menc_os2.split('@')[0]} 🥵`, contextInfo: {...DLchanneldl, mentionedJid: [menc_os2]}}, {quoted: selo})
break
...
await sendPoll(SystemDark, from, `*🤔 𝐕𝐎𝐂𝐄̂ 𝐏𝐑𝐄𝐅𝐄𝐑𝐄 ⧽*\n•\n> 1️⃣ - ${pergunta1}\n-\n> 2️⃣ - ${pergunta2}\n•\n⚡ 𝐄𝐒𝐂𝐎𝐋𝐇𝐀 𝐁𝐄𝐌...`, [ '✰ 𝐎𝐏𝐂̧𝐀̃𝐎 𝟏 ✰', '✰ 𝐎𝐏𝐂̧𝐀̃𝐎 𝟐 ✰' ]);
} catch (err) {
...
await sendPoll(SystemDark, from, `*❓𝑃𝐸𝑅𝐺𝑈𝑁𝑇𝐴 ⧽*\n\n> ${Never}\n\n✅ 𝐕𝐎𝐂𝐄̂ 𝐉𝐀 𝐎𝐔 𝐍𝐔𝐍𝐂𝐀? ❎`, [ `✰ 𝐄𝐔 𝐉𝐀 😳 ✰`, `✰ 𝐄𝐔 𝐍𝐔𝐍𝐂𝐀 👀 ✰` ]
);
} catch (err) {
...
await SystemDark.sendMessage(from, {image: {url: rnkgay}, caption: rankGay, contextInfo:{...DLchanneldl, mentionedJid: mentionsGay}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rankcasal}, caption: rankCasal, contextInfo:{...DLchanneldl, mentionedJid: mentionsCasal}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rankfalido}, caption: rankFalido, contextInfo:{...DLchanneldl, mentionedJid: mentionsFalido}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rankcu}, caption: rankCu, contextInfo:{...DLchanneldl, mentionedJid: mentionsCu}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rankbct}, caption: rankBCT, contextInfo:{...DLchanneldl, mentionedJid: mentionsBCT}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rnkgado}, caption: rankGado, contextInfo:{...DLchanneldl, mentionedJid: mentionsGado}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rnkcorno}, caption: rankCorno, contextInfo:{...DLchanneldl, mentionedJid: mentionsCorno}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {text: msgSuruba, contextInfo:{...DLchanneldl, mentionedJid: mentionsSuruba}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rnkgostoso}, caption: rankGostoso, contextInfo:{...DLchanneldl, mentionedJid: mentionsGostoso}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rnkgostosa}, caption: rankGostosa, contextInfo:{...DLchanneldl, mentionedJid: mentionsGostosa}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image: {url: rnknazista}, caption: rankNazista, contextInfo:{...DLchanneldl, mentionedJid: mentionsNazista}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkotaku}, caption: rankOtaku, contextInfo:{...DLchanneldl, mentionedJid: mentionsOtaku}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnksigma}, caption: rankSigma, contextInfo:{...DLchanneldl, mentionedJid: mentionsSigma}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkbeta}, caption: rankBeta, contextInfo:{...DLchanneldl, mentionedJid: mentionsBeta}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkbaiano}, caption: rankBaiano, contextInfo:{...DLchanneldl, mentionedJid: mentionsBaiano}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkbaiana}, caption: rankBaiana, contextInfo:{...DLchanneldl, mentionedJid: mentionsBaiana}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkcarioca}, caption: rankCarioca, contextInfo:{...DLchanneldl, mentionedJid: mentionsCarioca}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnklouco}, caption: rankLouco, contextInfo:{...DLchanneldl, mentionedJid: mentionsLouco}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnklouca}, caption: rankLouca, contextInfo:{...DLchanneldl, mentionedJid: mentionsLouca}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnksafada}, caption: rankSafada, contextInfo:{...DLchanneldl, mentionedJid: mentionsSafada}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnksafado}, caption: rankSafado, contextInfo:{...DLchanneldl, mentionedJid: mentionsSafado}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkmacaco}, caption: rankMacaco, contextInfo:{...DLchanneldl, mentionedJid: mentionsMacaco}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkmacaca}, caption: rankMacaca, contextInfo:{...DLchanneldl, mentionedJid: mentionsMacaca}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkputa}, caption: rankPuta, contextInfo:{...DLchanneldl, mentionedJid: mentionsPuta}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {image:{url: rnkpau}, caption: rankPau, contextInfo:{...DLchanneldl, mentionedJid: mentionsPau}}, {quoted: selo})
break
...
await SystemDark.sendMessage(from, {text: texto, contextInfo:{...DLchanneldl, mentionedJid: mentionsList}
}, {quoted: selo})
break;
}
...
fs.writeFileSync('./DADOS_SYSTEM/INFO_SYSTEM/media/countmsg.json', JSON.stringify(countMessage, null, 2));
...
await SystemDark.sendMessage(from,{text: resp, contextInfo: { ...DLchanneldl, mentionedJid: [alvo]}}, { quoted: selo });
break;
}
...
await SystemDark.sendMessage(from, { image: { url: namorar }, caption: texto, contextInfo: { ...DLchanneldl, mentionedJid: [menc_os2, sender] }}, { quoted: selo });
break;
}
...
await SystemDark.sendMessage(jidParceiro, { text: `*💔 ᴛᴇɴʜᴏ ᴜᴍᴀ ɴᴏᴛɪ́ᴄɪᴀ ᴛʀɪꜱᴛᴇ... ꜱᴇᴜ ᴘᴀʀᴄᴇɪʀᴏ(ᴀ) ᴀᴄᴀʙᴏᴜ ᴅᴇ ᴛᴇʀᴍɪɴᴀʀ ᴏ ɴᴀᴍᴏʀᴏ...😔*\n> *ɢᴜᴀʀᴅᴇ ᴏꜱ ʙᴏɴꜱ ᴍᴏᴍᴇɴᴛᴏꜱ, ᴍᴇꜱᴍᴏ ǫᴜᴇ ᴅᴏᴀ...🙇‍♂️*`, contextInfo: { ...DLchanneldl, mentionedJid: [sender, jidParceiro] }}, { quoted: selo });
namoro1.splice(D1, 1);
fs.writeFileSync('./DADOS_SYSTEM/func/namoro1.json', JSON.stringify(namoro1));
break;
}
...
try {
const ppimg = await SystemDark.profilePictureUrl(parceiro2, 'image')
await SystemDark.sendMessage(from, { image: { url: ppimg }, caption: texto, mentions: [parceiro1, parceiro2], contextInfo: { ...DLchanneldl, mentionedJid: [parceiro1, parceiro2] }}, { quoted: selo })
} catch {
await SystemDark.sendMessage(from, { image: { url: imgperfil }, caption: texto, mentions: [parceiro1, parceiro2], contextInfo: { ...DLchanneldl, mentionedJid: [parceiro1, parceiro2] }}, { quoted: selo })}
}
break
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`);
reply("Desativado com sucesso.");
} else {
const anaaleatorio = Math.floor(Math.random() * palavrasANA.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/anagrama/${from}.json`, JSON.stringify(palavrasANA[anaaleatorio]));
await SystemDark.sendMessage(from, {text: `🌟😲 Decifre a palavra embaralhada abaixo, qual será a ordem correta das letras?\n—\n• Palavra: *${shuffle(palavrasANA[anaaleatorio].palavraOriginal)}*\n• Dica: ${palavrasANA[anaaleatorio].dica}`}, {quoted: selo});
}
break;
...
const fileContent = fs.readFileSync("./SystemDark.js", "utf8");

const matches = fileContent.match(/case\s+(['"`])(.+?)\1/g) || [];

const comandos = [...new Set(
matches.map(cmd => {
return cmd.match(/case\s+['"`](.+?)['"`]/)[1];
})
)];

reply(`💮 ᴛᴏᴛᴀʟ ᴅᴇ ᴄᴏᴍᴀɴᴅᴏs: ${comandos.length}`);

} catch (e) {
console.log(e);
reply("❌ | ᴇʀʀᴏ ᴀᴏ ᴄᴏɴᴛᴀʀ ᴏs ᴄᴏᴍᴀɴᴅᴏs.");
}
}
break;
...
const fileContent = fs.readFileSync("./SystemDark.js", "utf-8");
...
console.log(e);
reply("ᴏᴄᴏʀʀᴇᴜ ᴜᴍ ᴇʀʀᴏ ᴀᴏ ᴏʙᴛᴇʀ ᴀs ᴄᴀsᴇs.");
}
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`);
reply("Desativado com sucesso.");
} else {
const animaisquiz = Math.floor(Math.random() * quizanimais.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/quiz-animais/${from}.json`, JSON.stringify(quizanimais[animaisquiz]));
wew = await getBuffer(`${quizanimais[animaisquiz].foto}`);
await SystemDark.sendMessage(from, {image: wew, caption: `🤔 Pergunta: ${quizanimais[animaisquiz].question}`}, {quoted: selo});
}
break;
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`);
reply("Desativado com sucesso.");
} else {
const whatMAle = Math.floor(Math.random() * whatMusicAr.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/wmusic/${from}.json`, JSON.stringify(whatMusicAr[whatMAle]));
await SystemDark.sendMessage(from, {text: `🎶🎧 𝐖𝐇𝐀𝐓 𝐌𝐔𝐒𝐈𝐂? 😱💡\n–\n${whatMusicAr[whatMAle].trechoMusic}\n–\n🤔😱 Qual música pertence o trecho apresentado acima?\n• Dica: ${whatMusicAr[whatMAle].dica}`}, {quoted: selo});
}
break;
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`);
reply("Desativado com sucesso.");
} else {
const garticquiz = Math.floor(Math.random() * garticArchives.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/gartic/${from}.json`, JSON.stringify(garticArchives[garticquiz]));
await SystemDark.sendMessage(from, {image: {url: `${garticArchives[garticquiz].imagem}`}, caption: `👩🏼‍🏫 - A resposta é representada por um(a): ${garticArchives[garticquiz].pergunta}\n📜 - A resposta supostamente começa com a(s) letra(s): "${garticArchives[garticquiz].letra_inicial}"\n🤔 - Contém traços? ${garticArchives[garticquiz].contem_traços}`}, {quoted: selo});
}
break;
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`);
reply("Desativado com sucesso.");
} else {
const engimaSolu = Math.floor(Math.random() * enigmaArchive.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/enigma/${from}.json`, JSON.stringify(enigmaArchive[engimaSolu]));
await SystemDark.sendMessage(from, {image: {url: enigma}, caption: `📜 - Resolva o seguinte enigma abaixo:\n—\n${enigmaArchive[engimaSolu].charada}\n–\n❓️ - *Não sabe a resposta?* _Peça ao adm do grupo para usar o comando *${prefix}revelarenigma* para revelar a resposta correta._`}, {quoted: selo});
}
break;
...
if (!isGroupAdmins) return reply(mess.onlyAdmins());
if(!isModoCoins) return reply(`*ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ sᴏ ᴘᴏᴅᴇ sᴇʀ ᴀᴛɪᴠᴏ ǫᴜᴀɴᴅᴏ ᴏ sɪᴛᴇᴍᴀ ${prefix}ᴍᴏᴅᴏᴄᴏɪɴs ᴇsᴛɪᴠᴇʀ ᴀᴛɪᴠᴏ 🤷‍♂️*`);
if (fs.existsSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`)) {
fs.unlinkSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`);
reply("Desativado com sucesso.");
} else {
const futebolquiz = Math.floor(Math.random() * quizFutebol.length);
fs.writeFileSync(`./DADOS_SYSTEM/grupos/games/quiz-futebol/${from}.json`, JSON.stringify(quizFutebol[futebolquiz]));
await SystemDark.sendMessage(from, {text: `💫⚽ 𝐐𝐔𝐈𝐙 𝐅𝐔𝐓𝐄𝐁𝐎𝐋 ⚽💫\n–\n*🗣️| Responda a pergunta mostrada abaixo:*\n• _${quizFutebol[futebolquiz].pergunta}_`}, {quoted: selo});
}
break;
...
┃֪࣪𝆺𝅥🪷 [ ${V_[0]} | ${V_[1]} | ${V_[2]} ] ◄
╠╾═✯═╼┈𑁁┈💮┈𑁁┈╾═✯═╼╮
┃֪࣪𝆺𝅥💎 𝐂𝐇𝐀𝐍𝐂𝐄𝐒 ➥ ${usuario.chances.cassino}/5
┃֪࣪𝆺𝅥🪷 𝐂𝐀𝐒𝐒𝐈𝐍𝐎 𝐃𝐈𝐀́𝐑𝐈𝐎
╰━━━━━━━━━━━━━━━━━━╯
...
let arquivoGartic = `./DADOS_SYSTEM/grupos/games/gartic/${from}.json`;

if (!fs.existsSync(arquivoGartic)) {
return reply("Não existe nenhuma partida atual do jogo neste grupo.");
}

let datenagramaa = JSON.parse(fs.readFileSync(arquivoGartic));

reply(`• Olá *${pushname}*, a resposta correta da afirmação era: *${datenagramaa.resposta}*\n• Envie a resposta apresentada acima para passar a próxima..`);
...
let caminhoEnigma = `./DADOS_SYSTEM/grupos/games/enigma/${from}.json`;

if (!fs.existsSync(caminhoEnigma)) return reply("Não existe nenhuma partida atual do jogo neste grupo.");

let eni1 = JSON.parse(fs.readFileSync(caminhoEnigma));

reply(`• Olá *${pushname}*, a resposta correta do enigma era: *${eni1.respostaEne}*\n• Envie a resposta apresentada acima para passar a próxima..`);
...
let mensagemRank = mensagemRank += `╭┈ׅׄ┉━━═✯𑁍💰𑁍✯═━━┉ֹׅ┈╮
┃֪࣪𝆺𝅥🪷 𝐑𝐀𝐍𝐊 • 𝐍-𝐂𝐎𝐈𝐍𝐒
╰━━━━━━━━━━━━━━━━━━╯\n\n`;
...
╭┈ׅׄ┉━━═✯𑁍💮𑁍✯═━━┉ֹׅ┈╮
┃֪࣪𝆺𝅥🪷 𝐔𝐒𝐄𝐑 ➥ @${usuario.id.split('@')[0]}
┃֪࣪𝆺𝅥💮 𝐒𝐀𝐋𝐃𝐎 ➥ ${usuario.coins} N-Coins
┃֪࣪𝆺𝅥🪷 𝐌𝐈𝐍𝐄𝐑𝐀𝐑 ➥ ${usuario.chances.minerar}/6
┃֪࣪𝆺𝅥💮 𝐂𝐀𝐒𝐒𝐈𝐍𝐎 ➥ ${usuario.chances.cassino}/5
╰┈ׅׄ┉━━═✯𑁍💮𑁍✯═━━┉ֹׅ┈╯\n\n`;
});
...
if(isGroupAdmins) return await SystemDark.sendMessage(from, {text: mess.antisRandomMessage()}, {quoted: selo})
if(IS_DELETE) {
setTimeout(async() => {
await SystemDark.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender}})
}, 500)
...
if(!JSON.stringify(groupMembers).includes(sender)) return
await SystemDark.groupParticipantsUpdate(from, [sender], 'remove')
await SystemDark.sendMessage(from, {text: `🗑${"\n".repeat(255)}🗑️\n✅️ A limpeza de chat foi concluída com sucesso.`, contextInfo: {forwardingScore: 500, isForwarded:true}});
await SystemDark.sendMessage(from, {text: 'Por favor, reporte aos adminstradores do grupo sobre o ocorrido.', mentions: groupAdmins})
}
}
}
...
info.participant?.toLowerCase().includes("bot") ||
info.pushName?.toLowerCase().includes("bot")
);
...
await SystemDark.groupParticipantsUpdate(
from,
[sender],
"remove"
);
...
await SystemDark.sendMessage(from,{
text:
`💮 @${sender.split("@")[0]} ꜰᴏɪ ʀᴇᴍᴏᴠɪᴅᴏ ᴘᴏʀ ᴘᴀʀᴇᴄᴇʀ ꜱᴇʀ ᴜᴍ ʙᴏᴛ`,
mentions:[sender]
},{quoted:selo});
...
if (isSystemDarkEscuta && isGroup && !info.key.fromMe) {
...
await SystemDark.sendMessage(from, { text: `*🎙️ ᴛʀᴀɴꜱᴄʀɪᴄ̧ᴀ̃ᴏ ᴅᴏ ᴀᴜ́ᴅɪᴏ 🎙️*\n-\n> *${textoFinal}*` }, { quoted: selo });
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/bomdia.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/boatarde.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/boanoite.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
if(budy2.includes("SystemDark")){
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/SystemDark.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/amor.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/oi.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/louca.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/porra.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/jack.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/gay.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/burro.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/gato.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/cu.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/depre.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/mulher.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/bct.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/risos.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/indio.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/fake.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/meme.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/macaco.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/macaco2.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/websescsu.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
const soundft = fs.readFileSync('./DADOS_SYSTEM/data/media/audios/sempai.mp3');
SystemDark.sendMessage(from, { audio: soundft, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
...
await SystemDark.sendMessage(from, {audio: tujuh, mimetype: "audio/mpeg", contextInfo: DLchanneldl
}, 
{
quoted: {
key: { participant: sender },
...
if(!JSON.stringify(groupMembers).includes(sender)) return 
SystemDark.groupParticipantsUpdate(from, [sender], 'remove');
setTimeout(() => {SystemDark.sendMessage(from, {delete: { remoteJid: from, fromMe: false, id: info.key.id, participant: sender}})}, 500)
}, 2000);
SystemDark.sendMessage(from, {text: mess.permissionDenied_rUser()}, {quoted: selo});
}
}
...
await SystemDark.sendMessage(from, {text: muehe}, {quoted: selo}).catch(async(e) => {
reply(mess.errorResponseSimi());
});
}
if (
isSystemDarkIA &&// recurso ligado
...
// SystemDark no começo | meio | fim (palavra isolada)
const chamouSystemDark =
/^SystemDark\b/.test(texto) ||
/\bSystemDark\b/.test(texto) ||
/\bSystemDark$/.test(texto);

if (!chamouSystemDark) return;
...
// remove apenas "SystemDark"
const pergunta = texto
.replace(/\bSystemDark\b/gi, "")
.replace(/\s+/g, " ")
.trim();
...
from,
{
document: fs.readFileSync("./DADOS_SYSTEM/selo.xlsx"),
mimetype:
...
Sou uma inteligência criada para responder com clareza, calma e precisão.
...
thumbnail: fs.readFileSync(
                "./DADOS_SYSTEM/INFO_SYSTEM/LOGOS/fotoselo.jpg"
),
sourceUrl: "https://tokito-apis.com.br"
}
}
},
{ quoted: info }
...
Você é SystemDark.
Responda de forma clara, objetiva e inteligente.
...
sourceUrl: "https://waguapis.online"
}
}
},
{ quoted: info }
...
console.error("Erro SystemDark IA:", err);
return;
}
}
...
╠╾═✯═╼┈𑁁┈𑁍┈𑁁┈╾═✯═╼╮
┃֪࣪𝆺𝅥🪷 𝐂𝐎𝐌𝐀𝐍𝐃𝐎 ➥ Inválido
┃֪࣪𝆺𝅥💮 𝐔𝐒𝐔𝐀́𝐑𝐈𝐎 ➥ @${sender.split("@")[0]}
┃֪࣪𝆺𝅥🪷 𝐃𝐈𝐆𝐈𝐓𝐀𝐃𝐎 ➥ ${pc}
┃֪࣪𝆺𝅥💮 𝐒𝐔𝐆𝐄𝐒𝐓𝐀̃𝐎 ➥ ${cmd || "Nenhum"}
┃֪࣪𝆺𝅥🪷 𝐒𝐈𝐌𝐈𝐋𝐀𝐑𝐈𝐃𝐀𝐃𝐄 ➥ ${porcentagem || 0}%
┃֪࣪𝆺𝅥💮 𝐀𝐉𝐔𝐃𝐀 ➥ ${prefix}menu
╠╾═✯═╼┈𑁁┈𑁍┈𑁁┈╾═✯═╼╯
╰┈ׅׄ┉━━═✯𑁍✯═━━┉ֹׅ┈╯`;
}
...
┃֪࣪𝆺𝅥💮 𝐏𝐑𝐄𝐅𝐈𝐗𝐎 ➥ [ ${prefix} ]
┃֪࣪𝆺𝅥🪷 𝐈𝐍𝐅𝐎 ☇ 
┃֪࣪𝆺𝅥.Utilize este prefixo para 
┃֪࣪𝆺𝅥acessar todos os comandos.
╠╾═✯═╼┈𑁁┈𑁍┈𑁁┈╾═✯═╼╯
╰┈ׅׄ┉━━═✯𑁍✯═━━┉ֹׅ┈╯`;
...
fs.writeFileSync("./DADOS_SYSTEM/usuarios/vip.json", JSON.stringify(vip));
if(y.infinito == false) {
if(y.dias > 1) {
y.dias -= 1;
fs.writeFileSync("./DADOS_SYSTEM/usuarios/vip.json", JSON.stringify(vip));
} else {
SystemDark.sendMessage(y.id, {text: `*sᴇᴜs ᴅɪᴀs ᴅᴇ ᴜsᴜᴀʀɪᴏ(ᴀ) ᴠɪᴘ ᴀᴄᴀʙᴀʀᴀᴍ 🙅‍♂️*`});
...
if(JSON.stringify(error).includes(API_KEY_SYSTEM_DARK2)) {} else if(String(error).includes("Erro: aborted")) {
...
module.exports = startSystemDark;
