...
if (fs.existsSync(dbPath)) {
    try {
        ...
        console.error('Erro ao ler rpg_data.json, utilizando estado inicial em memória:', err);
    }
}

// Salva as alterações de forma segura e atômica
...
export function ensureUserExists(userId, userName = 'Caçador Novato') {
...
export function getUserWeapons(userId) {
...
export function getEquippedWeapon(userId) {
...
export function getAllCatalogCards() {
...
export function saveActiveRaid(newRaid) {
...
export function modifyItemQuantity(userId, itemId, delta) {
...
export function createGuild(name, leaderId) {
...
export function addMarketListing(sellerId, itemType, itemId, price) {
    const id = Date.now();
...
export function removeMarketListing(offerId) {
    dbData.marketListings = dbData.marketListings.filter(m => m.id !== Number(offerId));
    saveDB();
}

export function addShadow(userId, name, rank, atk, hp) {
...
export function getUserShadows(userId) {
